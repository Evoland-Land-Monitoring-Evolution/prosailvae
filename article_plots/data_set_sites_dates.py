...
def get_tile_dates(dir):
    ...
    for image_file in image_files:
        ...
        tile_dates = pd.concat((tile_dates, pd.DataFrame({"tile":tile, "date":date}, index=[0])), ignore_index=True)
    return tile_dates
...
def tile_and_dates_plot(tile_dates):
    ...
    xtickslabels = [ax.get_xticklabels()[j].get_text() for j in range(len(ax.get_xticklabels()))]
    xtickspos = ax.get_xticks()
    plt.close('all')
    fig, ax = plt.subplots()
    for i, tile in enumerate(tiles):
        ax.axhline(i, color="k", zorder=0, lw=1)
        ax.scatter((scatterplots[i][:,0] - min_x)/(max_x - min_x), scatterplots[i][:,1])
...
def main():
    ...
    tikzplotlib.save("/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/tile_dates.tex")
    fig.savefig("/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/tile_acquisition_dates.svg")
    pass
