...
def main(rsr_file = "", res_dir=""):
    ...
    rsr_range = (int(rsr[0, 0].item() * 1000),
                        int(rsr[0, -1].item() * 1000))
    nb_lambdas = prospect_range[1] - prospect_range[0] + 1
    rsr_prospect = torch.zeros([rsr.shape[0], nb_lambdas]).to(device)
    rsr_prospect[0, :] = torch.linspace(prospect_range[0],
                                                prospect_range[1],
                                                nb_lambdas).to(device)
    rsr_prospect[1:, :-(prospect_range[1] - rsr_range[1])] = rsr[1:, (prospect_range[0] - rsr_range[0]):]
