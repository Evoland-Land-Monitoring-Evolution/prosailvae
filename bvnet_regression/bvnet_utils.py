...
def get_bvnet_dataloader(variable='lai', valid_ratio=0.05, batch_size=2048, s2_r=None, prosail_vars=None, 
                         max_samples=50000, psoil0=0.3):
...
def get_bvnet_biophyiscal_from_batch(batch, patch_size=32, sensor=None, ver=None, device='cpu', 
                                              lai_bvnet=None, ccc_bvnet=None, cwc_bvnet=None):
    if ver is None:
        if sensor is None:
            ver = "2"
        elif sensor =="2A":
    ...
    for i in range(patched_s2_r.size(0)):
        for j in range(patched_s2_r.size(1)):
            ...
            with torch.no_grad():
                lai = torch.clip(lai_bvnet.forward(s2_data.to(lai_bvnet.device), spatial_mode=True), min=0)
                ccc = torch.clip(ccc_bvnet.forward(s2_data.to(ccc_bvnet.device), spatial_mode=True), min=0) # torch.clip(cab_bvnet.forward(s2_data, spatial_mode=True), min=0) / torch.clip(lai, min=0.1)
                cwc = torch.clip(cwc_bvnet.forward(s2_data.to(cwc_bvnet.device), spatial_mode=True), min=0) # torch.clip(cw_bvnet.forward(s2_data, spatial_mode=True), min=0) / torch.clip(lai, min=0. 1)
...
def get_bvnet_biophyiscal_from_pixellic_batch(batch, sensor=None, ver=None, device='cpu', 
                                              lai_bvnet=None, ccc_bvnet=None, cwc_bvnet=None,):
    if ver is None:
        if sensor is None:
            ver = "2"
    ...
    with torch.no_grad():
        ...
        lai = torch.clip(lai_bvnet.forward(s2_data.to(lai_bvnet.device), spatial_mode=False), min=0)
        ccc = torch.clip(ccc_bvnet.forward(s2_data.to(ccc_bvnet.device), spatial_mode=False), min=0) 
        cwc = torch.clip(cwc_bvnet.forward(s2_data.to(cwc_bvnet.device), spatial_mode=False), min=0) 
    return lai, ccc, cwc