...
def configureEmissionTracker(parser):
    ...
    try:
        ...
        tracker = OfflineEmissionsTracker(country_iso_code="FRA", output_dir=os.path.join(parser.root_results_dir, os.pardir))
        tracker.start()
...
def main():
    ...
    if len(parser.root_results_dir)==0:
        res_dir = os.path.join(os.path.join(os.path.dirname(prosailvae.__file__), os.pardir),"results/")
    else:
    ...
    for fold_res_dir in fold_res_dirs:
        ...
        try:
            ...
            save_results(PROSAIL_VAE, fold_res_dir, data_dir, LOGGER_NAME=LOGGER_NAME)
