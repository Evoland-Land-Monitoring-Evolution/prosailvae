...
RSR of the sensor.
    ...
    def __init__(self,
        ...
        bands_loc = bands_loc if bands_loc is not None else torch.zeros((len(self.bands_rsr_idx)))
        bands_scale = bands_scale if bands_scale is not None else torch.ones((len(self.bands_rsr_idx)))
        ...
        if self.spectral_resolution > 1:
            self.s2norm_factor_n = self.s2norm_factor_n[:,:,:-1].reshape(1, len(self.bands_rsr_idx), -1, 
                                                                         self.spectral_resolution).mean(3) * self.spectral_resolution
