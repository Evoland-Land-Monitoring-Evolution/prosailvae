...
GROUP_PPRO_BV = [N, cab, car, cbrown, cw, cm, ant, prot, cbc, lai, lidfa, hspot, psoil, rsoil]
...
class S2Data():
    ...
    def init_from_data(self, data:S2IO) -> None:
        self.data = data
        self.features_name = [key for (key, item) in data.asdict().items() if item is not None]
    ...
    def to_tensor(self, features_name:str|None=None):
        if features_name is None:
            return torch.cat([self.data.asdict()[f] for f in self.features_name if self.data.asdict()[f] is not None], 
        ...
        return torch.cat([self.data.asdict()[f] for f in features_name], 
                         dim=self.features_dim)
    ...
    def normalize(self, loc, scale):
        ...
        return torch.cat([(self.to_tensor([f]) - loc.to_tensor([f])) / scale.to_tensor([f]) for f in self.features_name],
                         dim=self.features_dim)
    ...
    def denormalize(self, loc, scale):
        for norm_data in [loc, scale]:
            ...
            assert norm_data.features_dim == 0
            assert self.check_init_features(norm_data.features_name)
        return torch.cat([self.to_tensor([f]) * scale.to_tensor([f]) + loc.to_tensor([f]) for f in self.features_name],
                         dim=self.features_dim)
    
class S2R(S2Data):
    def __init__(self, data:S2Bands|torch.Tensor, features_name:str|None=None, features_dim:int=0) -> None:
        super().__init__(data=data, features_name=features_name, features_dim=features_dim)
    
    def init_from_tensor(self, data:torch.Tensor, features_name:str):
        data_dict = {}
        for i, f in enumerate(features_name):
            data_dict[f] = data.select(dim=self.features_dim, index=i).unsqueeze(dim=self.features_dim)
