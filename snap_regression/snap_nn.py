...
def get_SNAP_norm_factors(ver:str='2', variable='lai'):
    ...
    if ver == "2":
        ...
        if variable=="lai":
            ...
            variable_min = DenormSNAPCCCV2().ccc_min / (DenormSNAPLAIV2().lai_max - DenormSNAPLAIV2().lai_min)
            variable_max = DenormSNAPCCCV2().ccc_max / (DenormSNAPLAIV2().lai_max - DenormSNAPLAIV2().lai_min)
        elif variable == "cwc":
            variable_min = DenormSNAPCWCV2().cwc_min
            variable_max = DenormSNAPCWCV2().cwc_max
        elif variable == "cw":
            variable_min = DenormSNAPCWCV2().cwc_min / (DenormSNAPLAIV2().lai_max - DenormSNAPLAIV2().lai_min)
            variable_max = DenormSNAPCWCV2().cwc_max / (DenormSNAPLAIV2().lai_max - DenormSNAPLAIV2().lai_min)
        else:
        ...
        if variable == "lai":
            ...
            variable_min = DenormSNAPCCCV3B().ccc_min / (DenormSNAPLAIV3B().lai_max - DenormSNAPLAIV3B().lai_min)
            variable_max = DenormSNAPCCCV3B().ccc_max / (DenormSNAPLAIV3B().lai_max - DenormSNAPLAIV3B().lai_min)
        elif variable == "cwc":
            variable_min = DenormSNAPCWCV3B().cwc_min
            variable_max = DenormSNAPCWCV3B().cwc_max
        elif variable == "cw":
            variable_min = DenormSNAPCWCV3B().cwc_min / (DenormSNAPLAIV3B().lai_max - DenormSNAPLAIV3B().lai_min)
            variable_max = DenormSNAPCWCV3B().cwc_max / (DenormSNAPLAIV3B().lai_max - DenormSNAPLAIV3B().lai_min)
        else:
            raise NotImplementedError
    elif ver=="3A":
        ...
        if variable == "lai":
            ...
            variable_min = DenormSNAPCCCV3A().ccc_min / (DenormSNAPLAIV3A().lai_max - DenormSNAPLAIV3A().lai_min)
            variable_max = DenormSNAPCCCV3A().ccc_max / (DenormSNAPLAIV3A().lai_max - DenormSNAPLAIV3A().lai_min)
        elif variable == "cwc":
            variable_min = DenormSNAPCWCV3A().cwc_min
            variable_max = DenormSNAPCWCV3A().cwc_max
        elif variable == "cw":
            variable_min = DenormSNAPCWCV3A().cwc_min / (DenormSNAPLAIV3A().lai_max - DenormSNAPLAIV3A().lai_min)
            variable_max = DenormSNAPCWCV3A().cwc_max / (DenormSNAPLAIV3A().lai_max - DenormSNAPLAIV3A().lai_min)
        else:
            raise NotImplementedError
    else:
        raise NotImplementedError
...
class SnapNN(nn.Module):
    ...
    def __init__(self, device:str='cpu', ver:str="3A", variable='lai', third_layer=False):
        ...
        if not third_layer:
            layers = OrderedDict([
                    ('layer_1', nn.Linear(in_features=input_size, out_features=hidden_layer_size).float()),
                    ('tanh', nn.Tanh()),
                    ('layer_2', nn.Linear(in_features=hidden_layer_size, out_features=1).float())])
        else:
            layers = OrderedDict([
                    ('layer_0', nn.Linear(in_features=input_size, out_features=input_size).float()),
                    ('tanh', nn.Tanh()),
                    ('layer_1', nn.Linear(in_features=input_size, out_features=hidden_layer_size).float()),
                    ('tanh', nn.Tanh()),
                    ('layer_2', nn.Linear(in_features=hidden_layer_size, out_features=1).float())])
    ...
    def save_weights(self, dir, prefix=""):
        ...
        pd.DataFrame(weights_1).to_csv(os.path.join(dir, f"{prefix}{self.ver}_{self.variable}_weights_1.csv"), 
                                       header=False, index=False, mode="w")
        pd.DataFrame(weights_2).to_csv(os.path.join(dir, f"{prefix}{self.ver}_{self.variable}_weights_2.csv"), 
                                       header=False, index=False, mode="w")
        pd.DataFrame(bias_1).to_csv(os.path.join(dir, f"{prefix}{self.ver}_{self.variable}_bias_1.csv"), 
                                       header=False, index=False, mode="w")
        pd.DataFrame(bias_2).to_csv(os.path.join(dir, f"{prefix}{self.ver}_{self.variable}_bias_2.csv"), 
                                       header=False, index=False, mode="w")
        

    def load_weights(self, dir, prefix=""):
        weights_1 = torch.from_numpy(pd.read_csv(os.path.join(dir, f"{prefix}{self.ver}_{self.variable}_weights_1.csv"), 
                                                 header=None).values).float()
        weights_2 = torch.from_numpy(pd.read_csv(os.path.join(dir, f"{prefix}{self.ver}_{self.variable}_weights_2.csv"), 
                                                 header=None).values).float()
        bias_1 = torch.from_numpy(pd.read_csv(os.path.join(dir, f"{prefix}{self.ver}_{self.variable}_bias_1.csv"), 
                                              header=None).values).float()
        bias_2 = torch.from_numpy(pd.read_csv(os.path.join(dir, f"{prefix}{self.ver}_{self.variable}_bias_2.csv"), 
                                              header=None).values).float()
