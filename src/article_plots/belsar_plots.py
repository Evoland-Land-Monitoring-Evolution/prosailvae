...
def plot_sampling_dates(s2_dates=None):
    ...
    if s2_dates is not None:
        ...
        s2_levels = np.tile([2, 2, 2, 2, 2, 2], int(np.ceil(len(s2_dates) / 6)))[
            : len(s2_dates)
        ]
        ax.vlines(s2_dates, 0, s2_levels, color="tab:green")  # The vertical stems.
        ax.plot(
            s2_dates, np.zeros_like(s2_dates), "-o", color="k", markerfacecolor="w"
...
def get_belsar_sites_time_series(
    metrics,
    belsar_data_dir,
    site="W1",
    fig=None,
    ...
    for date in pd.unique(validation_df["date"]):
        ...
        if not len(sub_val_df[sub_val_df["Field ID"] == site]) == 0:
            ...
            lai.append(
                np.mean(sub_val_df[sub_val_df["Field ID"] == site]["lai"].values)
            )
            lai_std.append(
                np.std(sub_val_df[sub_val_df["Field ID"] == site]["lai"].values)
            )
            fields.append(site)
    ...
    ax.errorbar(
        all_metrics["Date"].values,
        all_metrics["LAI"].values,
        yerr=all_metrics["lai_std"],
        ecolor="k",
        capthick=1,
        fmt="o",
        linestyle="",
    ...
    if use_ref_metrics:
        ref_metrics = pd.DataFrame(
            {
                "Date": [datetime.strptime(d, "%Y-%m-%d") for d in measurement_dates],
                "LAI": lai,
                "lai_std": lai_std,
                "name": fields,
        ...
        ax.scatter(
            ref_metrics["Date"].values, ref_metrics["LAI"].values, label="Measurement"
        )
        ax.errorbar(
            ref_metrics["Date"].values,
            ref_metrics["LAI"].values,
            yerr=ref_metrics["lai_std"],
            ecolor="k",
            capthick=1,
            fmt="o",
            linestyle="",
...
def get_belsar_lai_vs_hspot(
    ...
    for site in sites:
        for date in pd.unique(validation_df["date"]):
            ...
            if not len(sub_val_df[sub_val_df["Field ID"] == site]) == 0:
                ...
                fields.append(site)
    site_metrics = metrics[metrics["name"] == site]
    all_metrics = site_metrics[
        ["lai_mean", "lai_sigma_mean", "hspot_mean", "hspot_sigma_mean", "name"]
    ].copy()
    ...
    ax.errorbar(
        all_metrics["LAI"].values,
        all_metrics["hspot"].values,
        xerr=all_metrics["lai_std"],
        yerr=all_metrics["hspot_std"],
        ecolor="k",
        capthick=1,
        fmt="o",
        linestyle="",
...
if __name__ == "__main__":
    ...
    fig, ax = plot_measurements_and_s2_dates(
        s2_dates=s2_dates,
