...
from bvnet_regression.bvnet_utils import get_bvnet_dataloader, initialize_bvnet
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data_snap_simu_dist/",
    )
    ...
    parser.add_argument("-s", dest="simulate_dataset", type=bool, default=False)

    parser.add_argument("-v", dest="validate_on_terrain", type=bool, default=False)

    parser.add_argument("-si", dest="sigma", type=float, default=3)
    return parser
...
def simulate_data_sets(
    ...
    lai_var_dist = VariableDistribution(
        low=lai_low, high=lai_high, scale=tg_sigma_0, loc=tg_mu_0, law="gaussian"
    )
    ...
    for i, mu in enumerate(tg_mu):
        for j, sigma in enumerate(tg_sigma):
            ...
            lai_var_dist = VariableDistribution(
                low=lai_low, high=lai_high, scale=sigma, loc=mu, law="gaussian"
            )
            save_dataset(
                save_dir,
                f"train_mu_{mu}_sigma_{sigma}_",
                nb_simus=n_samples_sub,
                rsr_dir=rsr_dir,
                bvnet_bands=False,
                noise=0.01,
                prosail_var_dist_type="new_v2",
...
def main():
    if socket.gethostname() == "CELL200973":
        args = [
            "-d",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/snap_distribution_data/",
            "-r",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/results/snap_distribution_validation/",
            "-e",
            "400",
            "-n",
            "2",
    ...
    if parser.simulate_dataset:
        simulate_data_sets(
            n_eval=n_eval,
            n_samples_sub=n_samples_sub,
            save_dir=data_dir,
            tg_mu=tg_mu,
            tg_sigma=tg_sigma,
            tg_mu_0=tg_mu_0,
            tg_sigma_0=tg_sigma_0,
    ...
    prosail_s2_sim = torch.load(
        os.path.join(data_dir, f"evaluation_prosail_s2_sim_refl.pt")
    )[:, bands]
    prosail_vars = torch.load(
        os.path.join(data_dir, f"evaluation_prosail_sim_vars.pt")
    )[:, vars]
    eval_loader, _, _, _ = get_bvnet_dataloader(
        variable="lai",
        valid_ratio=0.0,
    ...
    for i, mu in enumerate(tg_mu):
        ...
        for j, sigma in enumerate(tg_sigma):
            # scale_bv = sigma
            kl = kl_tntn(
                torch.as_tensor(tg_mu_0),
                torch.as_tensor(tg_sigma_0),
                torch.as_tensor(mu),
                torch.as_tensor(sigma),
                lower=torch.as_tensor(lai_low),
                upper=torch.as_tensor(lai_high),
            ).item()
            kl_grid[i, j] = kl
            prosail_s2_sim = torch.load(
                os.path.join(
                    data_dir, f"train_mu_{mu}_sigma_{sigma}_prosail_s2_sim_refl.pt"
                )
            )[:, bands]
            prosail_vars = torch.load(
                os.path.join(
                    data_dir, f"train_mu_{mu}_sigma_{sigma}_prosail_sim_vars.pt"
                )
            )[:, vars]
            train_loader, valid_loader, loc_bv, scale_bv = get_bvnet_dataloader(
                variable="lai",
                valid_ratio=0.05,
                batch_size=batch_size,
                s2_r=prosail_s2_sim,
            ...
            for n in range(parser.n_model_train):
                ...
                res_df_filename = os.path.join(
                    res_dir, "snap_distribution_regression_results.csv"
                )
                ...
                if parser.validate_on_terrain:
                    ...
                    df_results = get_belsar_x_frm4veg_lai_results(
                        belsar_results,
                        barrax_results,
                        barrax_2021_results,
                        wytham_results,
                    ...
                    rmse, _, _, _ = get_validation_global_metrics(
                        df_results, decompose_along_columns=["Campaign"], variable="lai"
                    )
                    lai_terrain_rmse = rmse["Campaign"][f"lai_rmse_all"].values[0]
                    model_df = pd.concat(
                        (model_df, pd.DataFrame({"terrain_rmse": [lai_terrain_rmse]})),
                        axis=1,
                    )
                    lai_terrain_rmse_list.append(lai_terrain_rmse)
                if not os.path.isfile(res_df_filename):
                    model_df.to_csv(
                        res_df_filename, header=model_df.columns, index=False
                    )
    ...
    if parser.validate_on_terrain:
        all_results_df = pd.concat(
            (model_df, pd.DataFrame({"terrain_rmse": lai_terrain_rmse_list})), axis=1
        )
    all_results_df.to_csv(
        os.path.join(res_dir, "snap_distribution_regression_results_all.csv"),
        index=False,
    )
    ...
    np.save(
        os.path.join(res_dir, "snap_distribution_regression_loss.npy"), valid_loss_grid
    )
