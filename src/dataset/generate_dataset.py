...
def correlate_with_lai_V2(
    lai, V, Vmin_0, Vmax_0, Vmin_lai_max, Vmax_lai_max, lai_max, lai_thresh=None
):
...
def correlate_sample_with_lai(
    sample,
    lai,
    param_dist: VariableDistribution,
    lai_corr_mode: str = "v2",
    lai_conv_override: bool | None = None,
    lai_max: float = 15,
    ...
    if lai_corr_mode == "v2":
        if param_dist.C_lai_min is not None:
            correlated_sample = correlate_with_lai_V2(
                ...
                param_dist.low,
                param_dist.high,
                param_dist.C_lai_min,
                param_dist.C_lai_max,
                lai_max=lai_max,
                lai_thresh=lai_thresh,
            )
    else:
        if lai is not None and (
            param_dist.lai_conv is not None or lai_conv_override is not None
        ):
            lai_conv = (
                lai_conv_override
                if lai_conv_override is not None
                else param_dist.lai_conv
            )
            if param_dist.loc is None:
                correlated_sample = correlate_with_lai_V1(
                    lai, sample, (param_dist.high - param_dist.low) / 2, lai_conv
                )
            else:
                correlated_sample = correlate_with_lai_V1(
                    lai, sample, param_dist.loc, lai_conv
                )
        else:
            correlated_sample = sample.copy()
...
def correlate_all_variables_with_lai(
    ...
    for variable, idx in variable_idx_dict.items():
        ...
        correlated_samples[:, idx] = correlate_sample_with_lai(
            samples[:, idx],
            samples[:, 6],
            variable_dist,
...
def simulate_reflectances(
    ...
    for i in range(n_full_batch):
        ...
        if noise > 0:
            ...
            sim_s2_r += add_noise
        prosail_s2_sim[
            i * n_samples_per_batch : (i + 1) * n_samples_per_batch, :
        ] = sim_s2_r
    if last_batch > 0:
        sim_s2_r = ssimulator(
            psimulator(
...
def save_jordi_like_data_set(
    ...
    correlated_prosail_vars = correlate_all_variables_with_lai(
        prosail_vars, prosail_var_dist, lai_corr_mode="v2", lai_thresh=10
    )
    ...
    for prospect_version in ["5", "D", "PRO"]:
        ...
        torch.save(
            torch.from_numpy(correlated_prosail_vars),
            os.path.join(
                data_dir,
                f"{data_file_prefix}PROSPECT{prospect_version}_corr_v2_prosail_sim_vars.pt",
            ),
        )
        torch.save(
            ...
            os.path.join(
                data_dir,
                f"{data_file_prefix}PROSPECT{prospect_version}_corr_v2_prosail_s2_sim_refl.pt",
            ),
        )
        torch.save(
            norm_mean,
            os.path.join(
                data_dir,
                f"{data_file_prefix}PROSPECT{prospect_version}_corr_v2_norm_mean.pt",
            ),
        )
        torch.save(
            norm_std,
            os.path.join(
                data_dir,
                f"{data_file_prefix}PROSPECT{prospect_version}_corr_v2_norm_std.pt",
            ),
        )
        torch.save(
            cos_angles_loc,
            os.path.join(
                data_dir,
                f"{data_file_prefix}PROSPECT{prospect_version}_corr_v2_angles_loc.pt",
            ),
        )
        torch.save(
            cos_angles_scale,
            os.path.join(
                data_dir,
                f"{data_file_prefix}PROSPECT{prospect_version}_corr_v2_angles_scale.pt",
            ),
        )
        torch.save(
            idx_loc,
            os.path.join(
                data_dir,
                f"{data_file_prefix}PROSPECT{prospect_version}_corr_v2_idx_loc.pt",
            ),
        )
        torch.save(
            idx_scale,
            os.path.join(
                data_dir,
                f"{data_file_prefix}PROSPECT{prospect_version}_corr_v2_idx_scale.pt",
            ),
        )
    return


def save_prosail_data_set_with_all_prospect_versions(
    data_dir,
    data_file_prefix,
    rsr_dir,
    nb_simus,
    ...
    for lai_corr_mode in ["v1", "v2"]:
        ...
        for prospect_version in ["5", "D", "PRO"]:
            ...
            (
                ...
                torch.from_numpy(prosail_s2_sim).float().transpose(1, 0),
                mode="quantile",
            )
            torch.save(
                torch.from_numpy(correlated_prosail_vars),
                os.path.join(
                    data_dir,
                    f"{data_file_prefix}PROSPECT{prospect_version}_corr_{lai_corr_mode}_prosail_sim_vars.pt",
                ),
            )
            torch.save(
                torch.from_numpy(prosail_s2_sim),
                os.path.join(
                    data_dir,
                    f"{data_file_prefix}PROSPECT{prospect_version}_corr_{lai_corr_mode}_prosail_s2_sim_refl.pt",
                ),
            )
            torch.save(
                norm_mean,
                os.path.join(
                    data_dir,
                    f"{data_file_prefix}PROSPECT{prospect_version}_corr_{lai_corr_mode}_norm_mean.pt",
                ),
            )
            torch.save(
                norm_std,
                os.path.join(
                    data_dir,
                    f"{data_file_prefix}PROSPECT{prospect_version}_corr_{lai_corr_mode}_norm_std.pt",
                ),
            )
            torch.save(
                cos_angles_loc,
                os.path.join(
                    data_dir,
                    f"{data_file_prefix}PROSPECT{prospect_version}_corr_{lai_corr_mode}_angles_loc.pt",
                ),
            )
            torch.save(
                cos_angles_scale,
                os.path.join(
                    data_dir,
                    f"{data_file_prefix}PROSPECT{prospect_version}_corr_{lai_corr_mode}_angles_scale.pt",
                ),
            )
            torch.save(
                idx_loc,
                os.path.join(
                    data_dir,
                    f"{data_file_prefix}PROSPECT{prospect_version}_corr_{lai_corr_mode}_idx_loc.pt",
                ),
            )
            torch.save(
                idx_scale,
                os.path.join(
                    data_dir,
                    f"{data_file_prefix}PROSPECT{prospect_version}_corr_{lai_corr_mode}_idx_scale.pt",
                ),
            )
...
def save_bvnet_dataset(
    ...
    if nb_test_samples > 0:
        ...
        torch.save(
            torch.from_numpy(test_prosail_vars),
            os.path.join(bvnet_data_dir, "/bvnet_test_prosail_sim_vars.pt"),
        )
        torch.save(
            torch.from_numpy(test_prosail_s2_sim),
            os.path.join(bvnet_data_dir, "bvnet_test_prosail_s2_sim_refl.pt"),
        )
    ...
    torch.save(
        torch.from_numpy(train_prosail_vars),
        os.path.join(bvnet_data_dir, "bvnet_prosail_sim_vars.pt"),
    )
    torch.save(
        torch.from_numpy(train_prosail_s2_sim),
        os.path.join(bvnet_data_dir, "bvnet_prosail_s2_sim_refl.pt"),
    )

    norm_mean, norm_std = get_refl_normalization(train_prosail_s2_sim)
    torch.save(
        torch.from_numpy(norm_mean), os.path.join(bvnet_data_dir, "bvnet_norm_mean.pt")
    )
    torch.save(
        torch.from_numpy(norm_std), os.path.join(bvnet_data_dir, "bvnet_norm_std.pt")
    )
    cos_angle_min = torch.tensor(
        [0.342108564072183, 0.979624800125421, -1.0000]
    )  # sun zenith, S2 senith, relative azimuth
...
def save_lai_ccc_bvnet_dataset(
    data_dir,
    rsr_dir,
    noise=0,
    lai_corr=True,
...
def get_data_generation_parser():
    ...
    parser.add_argument(
        "-m", dest="lai_corr_mode", help="co-distribution mode", type=str, default="v2"
    )
    ...
    parser.add_argument(
        "-psa",
        dest="simulate_with_all_prospect",
...
if __name__ == "__main__":
    if socket.gethostname() == "CELL200973":
        args = [
            ...
            #   "-w", "True",
            "-d",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/sim_data_corr_v2_like_jordi/",
            "-dt",
            "new_v2",
            "-n",
            "42000",
    ...
    if parser.simulate_with_all_prospect:
        save_prosail_data_set_with_all_prospect_versions(
            data_dir,
            parser.file_prefix,
            parser.rsr_dir,
            parser.n_samples,
            parser.noise,
            bvnet_bands=parser.bvnet_bands,
            uniform_mode=False,
            prosail_var_dist_type=parser.dist_type,
            lai_thresh=lai_thresh,
        )
    if parser.simulate_like_jordi:
        save_jordi_like_data_set(
            data_dir,
            parser.file_prefix,
            parser.rsr_dir,
            parser.n_samples,
