...
TILES_BB = {
    ...
    "31UFS": {
        "bb_left_top": [[527850, 6586729], [549787, 6559371]],
        "crs": "epsg:32631",
    },
    "31UDP": {
        "bb_left_top": [[337026, 6198482], [217851, 6145959]],
        "crs": "epsg:32631",
    },
    ...
    "33TWF": {
        "bb_left_top": [[1741039, 5049233], [1732470, 4977074]],
        "crs": "epsg:32633",
    },  # [1736870, 4978840]
    ...
    "30SWJ": {
        "bb_left_top": [[-212019, 4739549], [-216509, 4738443], [-236810, 4741317]],
        "crs": "epsg:32630",
    },
}
...
def main():
    args = ["-t", "30TUM", "-m", "0.01", "-o", "/home/yoel/Téléchargements/tile_s2"]
    parser = get_parser().parse_args(args)
    # parser = get_parser().parse_args()
    # for tile in ["30SVG", "30STE", "33SVB", "31UCS"]:
    list_product = [
        "SENTINEL2A_20170423-104254-989_L2A_T31TCJ_D",
    ...
    if not download:
        for tile in ["30SWJ"]:
            ...
            tiles, bb_list = get_sites_bb(
                TILES_BB, tiles=[tile], in_crs="epsg:3857", size=5120
            )
    else:
        for tile in ["30SWJ"]:  # "31TCJ",
            ...
            tensor_dir = os.path.join(
                "/home/yoel/Téléchargements/tile_s2/torch_files", "T" + tile
            )
            list_tile_product = []
            ...
            for product_name in list_tile_product:
                for roi, top_left in enumerate(TILES_BB[tile]["bb_left_top"]):
                    ...
                    product_tensor = theia_product_to_tensor(
                        os.path.join(tile_dir, product_name),
                        product_dir,
                        part_loading=1,
                        top_left=top_left,
                        n_pixels=512,
                    ...
                    if not product_tensor.isnan().any():
                        ...
                        fig.savefig(
                            os.path.join(tensor_dir, product_name + f"_ROI_{roi}.png")
                        )
                        print(
                            f"Saving tensor file at {os.path.join(tensor_dir, f'{product_name}_ROI_{roi}.pth')}"
                        )
                        torch.save(
                            product_tensor,
                            os.path.join(tensor_dir, f"{product_name}_ROI_{roi}.pth"),
                        )
                    else:
                        print("NaN tensor !")
