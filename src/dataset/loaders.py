...
def split_train_valid_by_fid(labels, ts_ids, valid_size=0.1, seed=42):
...
def get_s2loader(
    ...
    if valid_ratio is None:
        ...
        ids_train, ids_valid = split_train_valid_by_fid(
            sub_labels, ts_ids, valid_size=0.1
        )
...
def load_train_valid_ids(k=None, n=None, file_prefix="s2_", data_dir=None):
    ...
    if k is None or n is None:
        ...
        train_valid_ids = torch.load(
            data_dir + f"/{file_prefix}train_valid_ids_{k}_{n}.pt"
        )
...
def patchify_2D_tensor(tensor_2D, data_size=10, patch_size=32):
    ...
    patchified_tensor = (
        tensor_2D.reshape(batch_size, patch_size, patch_size, data_size)
        .transpose(2, 3)
        .transpose(1, 2)
    )
...
def get_loaders_from_image(
    ...
    s2_r_patches = patchify(s2_r, patch_size=patch_size, margin=0).reshape(
        -1, len(bands), patch_size, patch_size
    )
    s2_a_patches = patchify(angles, patch_size=patch_size, margin=0).reshape(
    ...
    valid_idx = perms[
        torch.arange(
            int(train_ratio * n_patches), int((valid_ratio + train_ratio) * n_patches)
        )
    ]
    test_idx = perms[
        torch.arange(int((valid_ratio + train_ratio) * n_patches), n_patches)
    ]
    if concat:
        train_dataset = TensorDataset(
            torch.cat(
                (s2_r_patches[train_idx, ...], s2_a_patches[train_idx, ...]), axis=1
            )
        )
        valid_dataset = TensorDataset(
            torch.cat(
                (s2_r_patches[valid_idx, ...], s2_a_patches[valid_idx, ...]), axis=1
            )
        )
        test_dataset = TensorDataset(
            torch.cat(
                (s2_r_patches[test_idx, ...], s2_a_patches[test_idx, ...]), axis=1
            )
        )
    else:
        train_dataset = TensorDataset(
            s2_r_patches[train_idx, ...], s2_a_patches[train_idx, ...]
        )
        valid_dataset = TensorDataset(
            s2_r_patches[valid_idx, ...], s2_a_patches[valid_idx, ...]
        )
        test_dataset = TensorDataset(
            s2_r_patches[test_idx, ...], s2_a_patches[test_idx, ...]
        )
    train_loader = DataLoader(
...
def get_loader_from_patches(
    ...
    if plot_distribution:
        pass
    print(
        f"sample mean of bands values in loader: {patches[0,:10,:,:].reshape(10,-1).mean(1).cpu()}"
    )
...
def get_bands_norm_factors_from_loaders(
    loader, bands_dim=1, max_samples=10000, n_bands=10, mode="mean"
):
    ...
    with torch.no_grad():
        ...
        if mode == "mean":
            ...
            norm_std = torch.quantile(
                s2_r_samples, q=torch.tensor(0.95), dim=1
            ) - torch.quantile(s2_r_samples, q=torch.tensor(0.05), dim=1)
    return norm_mean, norm_std
