        ...
        if os.path.isdir(tile_dir):
            ...
            for tile_file in tile_files:
                ...
                if tile_file[-4:] == ".pth":
                    ...
                    if id in id_list:
                        print(
                            f"Already an image with : {sensor}, {date}, {tile}, {roi}"
                        )
                    else:
                        id_list.append(id)
...
def get_rectangle_bounds_from_patch_info(
    patch_idx, patch_size=128, image_size=(512, 512)
):
...
def plot_patch_rectangles(ax, patch_idxs, patch_size, image_size, color="red"):
    for idx in patch_idxs:
        xmin, xmax, ymin, ymax = get_rectangle_bounds_from_patch_info(
            idx, patch_size=patch_size, image_size=image_size
        )
        ax.add_patch(
            Rectangle(
                (xmin, ymin),
...
def get_train_valid_test_patch_tensors(
    data_dir,
    ...
    tensor_files, file_info = get_images_path(
        data_dir,
    ...
    for i, tensor_file in enumerate(tensor_files):
        ...
        perms = torch.randperm(
            patches.size(0), generator=g_cpu
        )  # For image tensor with identical sizes (i.e. the same sites) permutation will always be the same

        train_patches, nan_flag_1 = get_clean_patch_tensor(
            patches[perms[:n_train], ...], cloud_mask_idx=10, reject_mode="all"
        )
        valid_patches, nan_flag_2 = get_clean_patch_tensor(
            patches[perms[n_train : n_train + n_valid], ...],
            cloud_mask_idx=10,
            reject_mode="all",
        )
        test_patches, nan_flag_3 = get_clean_patch_tensor(
            patches[perms[n_train + n_valid :], ...],
            cloud_mask_idx=10,
            reject_mode="all",
        )
        ...
        if len(train_patches) > 0:
            ...
            train_patch_info += (
                [info] * n_train * (large_patch_size // train_patch_size) ** 2
            )

        if len(valid_patches) > 0:
            valid_clean_patches.append(valid_patches)
            valid_patch_info += (
                [info] * n_valid * (large_patch_size // train_patch_size) ** 2
            )
        ...
        if res_dir is not None:
            ...
            plot_patch_rectangles(
                ax,
                perms[:n_train],
                patch_size=large_patch_size,
                image_size=(
                    (image_tensor.size(1) // large_patch_size) * large_patch_size,
            ...
            fig.savefig(
                os.path.join(
                    res_dir,
                    f"{file_info[i][0]}_{file_info[i][1]}_{file_info[i][2]}_{file_info[i][3]}.png",
                )
            )
            plt.close("all")
    # raise NotImplementedError
    ...
    valid_clean_patches = patchify(
        unpatchify(valid_clean_patches.unsqueeze(0)), patch_size=train_patch_size
    ).reshape(-1, image_tensor.size(0), train_patch_size, train_patch_size)
    ...
    train_clean_patches = patchify(
        unpatchify(train_clean_patches.unsqueeze(0)), patch_size=train_patch_size
    ).reshape(-1, image_tensor.size(0), train_patch_size, train_patch_size)
...
def get_bands_norm_factors_from_patches(patches, n_bands=10, mode="mean"):
    ...
    with torch.no_grad():
        ...
        spectral_idx = (
            get_spectral_idx(patches[:, :n_bands, ...], bands_dim=1)
            .permute(1, 0, 2, 3)
            .reshape(4, -1)
        )
...
def main():
    if socket.gethostname() == "CELL200973":
        args = [
            "-d",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/torch_files/",
            "-o",
    ...
    if parser.theia:
        ...
        valid_files = None
        invalid_files = [
            "SENTINEL2A_20161001-110717-761_L2A_T30SVJ_D_V1-4.pth",
    ...
    (
        ...
        valid_patch_info,
        test_patch_info,
    ) = get_train_valid_test_patch_tensors(
    ...
    if plot_test:
        for i in range(test_patches.size(0)):
            ...
            fig.savefig(
                os.path.join(
                    parser.output_dir, f"test_{info[0]}_{info[1]}_{info[2]}.png"
                )
            )
            plt.close("all")
        pass
    ...
    rel_azi = (
        train_patches[:, 12, 0, 0] - train_patches[:, 14, 0, 0]
    )  # Sun azimuth - joint azimuth
    s2_a = torch.cat(
        (joint_zen.unsqueeze(1), sun_zen.unsqueeze(1), rel_azi.unsqueeze(1)), 1
    )
    s2_a_rad = torch.deg2rad(s2_a)
    s2_a_cos_sin = torch.cos(s2_a_rad)
    spectral_idx = (
        get_spectral_idx(train_patches[:, :10, ...], bands_dim=1)
        .permute(1, 0, 2, 3)
        .reshape(4, -1)
    )
    perm = torch.randperm(spectral_idx.size(1))
    pair_plot(
        spectral_idx.permute(1, 0)[perm[:1000000], :],
        tensor_2=None,
        features=["NDVI", "NDII", "ND_lma", "LAI_savi"],  # "mNDVI750", "CRI2",
        res_dir=parser.output_dir,
        filename="spectral_idx_pairplot.png",
    )
    pair_plot(
        s2_a_cos_sin,
        tensor_2=None,
        features=["Joint Zenith", "Sun Zenith", "Relative Azimuth"],
        res_dir=parser.output_dir,
        filename="angles_pairplot.png",
    )
    s2_a = torch.cat(
        (
            sun_zen.unsqueeze(1),
            joint_zen.unsqueeze(1),
            sun_azi.unsqueeze(1),
            joint_azi.unsqueeze(1),
        ),
        1,
    )
    pair_plot(
        s2_a,
        tensor_2=None,
        features=["Sun Zenith", "S2 Zenith", "Sun Azimuth", "S2 Azimuth"],
        res_dir=parser.output_dir,
        filename="angles_deg_pairplot.png",
    )
    ...
    (
        ...
        idx_norm_mean,
        idx_norm_std,
    ) = get_bands_norm_factors_from_patches(train_patches, mode=mode)
