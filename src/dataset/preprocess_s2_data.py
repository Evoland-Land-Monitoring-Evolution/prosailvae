...
def plot_patch(s2_r, idx=1):
    ...
    s2_r_n = normalize_patch_for_plot(
        s2_r[:, :3, :, :].permute(0, 2, 3, 1), sr_min=None, sr_max=None
    )
...
def analyse_angles(loader, max_batch=100):
    ...
    fig, ax = plt.subplots(dpi=100)
    im = (
        (s2_x.squeeze()[:1, :, :].permute((1, 2, 0)) / 10000).flip(2).cpu().numpy()
        * 255
    ).astype("int")
...
def main(
    tensors_dir="/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/real_data/torchfiles/",
    ...
    train_loader, _, _ = get_mmdc_loaders(
        tensors_dir=tensors_dir, batch_par_epoch=batch_par_epoch
    )
