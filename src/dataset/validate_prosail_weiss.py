...
def plot_bands_scatter_plots(
    s2_r, s2_r_sim, vars_names=["B3", "B4", "B5", "B6", "B7", "B8A", "B11", "B12"]
):
...
def compare_bvnet_w_simulations(prosail_var_bvnet, prosail_var_simu):
    fig, ax = plot_param_compare_dist(
        prosail_var_simu,
        prosail_var_bvnet,
    ...
    fig.savefig(
        "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/results/validation/sim_vs_bvnet_vers.png"
    )
    return


def lognormal_pdf(x, mu, sigma):
    return np.exp(-0.5 * (np.log(x) - mu) ** 2 / sigma**2) / (
...
def main():
    ...
    ssimulator = SensorSimulator(
        rsr_dir + "/sentinel2.rsr", bands=[2, 3, 4, 5, 6, 8, 11, 12]
    )
    ...
    for i in trange(prosail_vars_lai4.shape[0]):
        s2_r_sim[i, :] = ssimulator(
            psimulator(torch.from_numpy(prosail_vars_lai4[i, :]).view(1, -1).float())
        ).numpy()
        # plt.figure()
    ...
    fig, axs = plot_bands_scatter_plots(
        s2_r_lai4,
        s2_r_sim,
        vars_names=["B3", "B4", "B5", "B6", "B7", "B8A", "B11", "B12"],
    )
