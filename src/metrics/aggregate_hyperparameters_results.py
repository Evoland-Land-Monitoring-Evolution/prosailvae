...
def main():
    ...
    for i, dir_name in enumerate(res_dirs):
        ...
        if not os.path.isdir(root_res_dir + dir_name + "/fold_results/"):
            ...
            test_loss = pd.read_csv(
                root_res_dir + dir_name + "/fold_results/all_losses.csv", index_col=[0]
            ).values.reshape(-1)
            ...
            aer_percentiles = torch.load(
                root_res_dir + dir_name + "/fold_results/aer_percentiles.pt"
            )
            are_percentiles = torch.load(
                root_res_dir + dir_name + "/fold_results/are_percentiles.pt"
            )

            if len(test_loss) < n_folds:
                print(
                    f"WARNING : not all folds were available in experiment {dir_name}"
                )
                diff_size = n_folds - len(test_loss)
                test_loss = np.concatenate(
                    [test_loss, VALUE_ERROR * np.ones(diff_size)]
                )
                picp = torch.cat(
                    (
                        picp,
                        VALUE_ERROR
                        * torch.ones(diff_size, len(alpha_pi), len(PROSAILVARS)),
                    ),
                    dim=0,
                )
                mpiw = torch.cat(
                ...
                mae = torch.cat(
                    (mae, VALUE_ERROR * torch.ones(diff_size, len(PROSAILVARS))), dim=0
                )
                maer = torch.cat(
                    (maer, VALUE_ERROR * torch.ones(diff_size, len(PROSAILVARS))), dim=0
                )
                lat_nll = torch.cat(
                    (lat_nll, VALUE_ERROR * torch.ones(diff_size, len(PROSAILVARS))),
                    dim=0,
                )
                aer_percentiles = torch.cat(
                    (
                        aer_percentiles,
                        VALUE_ERROR * torch.ones(diff_size, 5, len(PROSAILVARS)),
                    ),
                    dim=0,
                )
                are_percentiles = torch.cat(
                    (
                        are_percentiles,
                        VALUE_ERROR * torch.ones(diff_size, 5, len(PROSAILVARS)),
                    ),
    ...
    pd.DataFrame(data=all_test_losses.cpu().squeeze().numpy()).to_csv(
        gathered_res_dir + "/all_losses.csv"
    )
    pass
