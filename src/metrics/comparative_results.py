...
from dataset.project_s2_dataset import (
    load_cyclical_data_set,
    save_common_cyclical_dataset,
)
from metrics.metrics_utils import regression_metrics
from metrics.prosail_plots import (
    patch_validation_reg_scatter_plot,
    plot_belsar_metrics,
    plot_patches,
    regression_plot,
)
...
from prosailvae.ProsailSimus import BANDS, get_bands_idx
from utils.image_utils import crop_s2_input, get_encoded_image_from_batch
from utils.utils import load_dict, load_standardize_coeffs, save_dict
from validation.belsar_validation import (
    interpolate_belsar_metrics,
    save_belsar_predictions,
    save_bvnet_belsar_predictions,
)
from validation.frm4veg_validation import (
    BARRAX_2021_FILENAME,
    BARRAX_FILENAMES,
    WYTHAM_FILENAMES,
    get_frm4veg_results_at_date,
    interpolate_frm4veg_pred,
    load_frm4veg_data,
)
...
def get_parser():
    ...
    parser.add_argument(
        "-m",
        dest="model_dict_path",
        help="path to model dict file",
        type=str,
        default="",
    )
    parser.add_argument(
        "-d", dest="data_dir", help="path to model dict file", type=str, default=""
    )
    parser.add_argument(
        "-rsr",
        dest="rsr_dir",
    ...
    parser.add_argument(
        "-r", dest="res_dir", help="path to results directory", type=str, default=""
    )
    parser.add_argument(
        "-sp",
        dest="save_projected",
    ...
    for model_name, model_info in model_dict.items():
        if model_info["type"] == "simvae":
            ...
            model = load_prosail_vae_with_hyperprior(
                pv_config=pv_config, pv_config_hyper=None, logger_name="No logger"
            )
...
def get_model_results(
    model_dict: dict, test_loader, info_test_data, max_patch=50, mode="lat_mode"
):
    """
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(test_loader)):
            ...
            for model_name, model_info in model_dict.items():
                ...
                current_patch_results[model_name] = {
...
def regression_pair_plot(scatter_dict, global_lim):
    ...
    """
    g = sns.pairplot(
        pd.DataFrame(data=scatter_dict),
        kind="reg",
        diag_kind="kde",
        plot_kws=dict(
...
def plot_frm4veg_results_comparison(
    model_dict, model_results, data_dir, filename, res_dir=None, prefix="", margin=0.02
):
    for variable in ["lai", "lai_eff", "ccc", "ccc_eff"]:
        n_models = len(model_dict) + 1
        fig, axs = plt.subplots(
            nrows=1, ncols=n_models, dpi=150, figsize=(6 * n_models, 6)
        )
        gdf, _, _, xcoords, ycoords = load_frm4veg_data(
            data_dir, filename, variable=variable
        )
        gdf = gdf.iloc[:51]
        xmin = min(
            np.min(gdf[variable].values),
            np.min(model_results["SNAP"][variable].reshape(-1)),
        )
        xmax = max(
            np.max(gdf[variable].values),
            np.max(model_results["SNAP"][variable].reshape(-1)),
        )
        ...
        for i, (model_name, model_info) in enumerate(model_dict.items()):
            ...
            pred_at_site = model_results[model_name][variable]
            fig, ax, g = patch_validation_reg_scatter_plot(
                gdf,
                pred_at_site=pred_at_site,
                variable=variable,
                fig=fig,
                ax=axs[i],
                legend=True,
        ...
        if res_dir is not None:
            fig.savefig(
                os.path.join(res_dir, f"{prefix}{variable}_{filename}_validation.png")
            )


def plot_lai_validation_comparison(
    model_dict,
    model_results,
    res_dir=None,
    prefix="",
    ...
    if res_dir is not None:
        fig.savefig(
            os.path.join(res_dir, f"{prefix}_{variable}_validation.png"),
            transparent=False,
        )


def get_belsar_validation_results(
    model_dict: dict, belsar_dir, res_dir, method="closest", mode=None, get_error=True
):
    model_results = {}
    for _, (model_name, model_info) in enumerate(model_dict.items()):
        model_results[model_name] = interpolate_belsar_metrics(
            belsar_data_dir=belsar_dir,
    ...
    model_results["SNAP"] = interpolate_belsar_metrics(
        belsar_data_dir=belsar_dir,
        belsar_pred_dir=res_dir,
        file_suffix="_SNAP",
        method=method,
...
def plot_belsar_validation_results_comparison(
    model_dict, model_results, res_dir=None, suffix="", margin=0.02
):
    n_models = len(model_dict) + 1

    xmin = min(
        np.min(model_results["SNAP"]["parcel_lai_mean"]),
        np.min(model_results["SNAP"]["lai_mean"]),
    )
    xmax = max(
        np.max(model_results["SNAP"]["parcel_lai_mean"]),
        np.max(model_results["SNAP"]["lai_mean"]),
    )
    for i, (model_name, model_info) in enumerate(model_dict.items()):
        # sub_variable = "lai" if variable in ["lai", "lai_eff"] else "ccc"
    ...
    fig, axs = plt.subplots(
        nrows=1, ncols=n_models - 1, dpi=150, figsize=(6 * n_models, 6)
    )
    for i, (model_name, model_info) in enumerate(model_dict.items()):
        # sub_variable = "lai" if variable in ["lai", "lai_eff"] else "ccc"
        metrics = model_results[model_name]
...
def get_training_metrics_df(
    ...
    for i, (model_name, model_info) in enumerate(model_dict.items()):
        training_metrics["rmse"].append(
            rmse_dict[variable][model_name]["Campaign"]["All"].values[0]
        )
        training_metrics["picp"].append(
            picp_dict[variable][model_name]["Campaign"]["All"].values[0]
        )
        training_metrics["mestdr"].append(
            mestdr_dict[variable][model_name]["Campaign"]["All"].values[0]
        )
...
def plot_comparative_results(
    ...
    if len(model_dict) < 7:
        for i in range(all_s2_r.size(0)):
            ...
            fig, _ = plot_patches(
                patch_list=[all_s2_r[i, ...]]
                + [
                    model_info["reconstruction"][i, ...]
            ...
            if res_dir is not None:
                fig.savefig(
                    os.path.join(
                        res_dir_i,
                        f"{i}_{info[1]}_{info[2]}_patch_reconstructions_rgb.png",
                    )
                )

            fig, _ = plot_patches(
                patch_list=[all_s2_r[i, torch.tensor([8, 3, 6]), ...]]
                + [
            ...
            if res_dir is not None:
                fig.savefig(
                    os.path.join(
                        res_dir_i, f"{i}_{info[1]}_{info[2]}_patch_rec_B8B5B11.png"
                    )
                )
            models_errs = [
                (all_s2_r[i, ...] - model_info["reconstruction"][i, ...])
                .abs()
                .mean(0)
                .unsqueeze(0)
                for _, model_info in model_dict.items()
            ...
            fig, _ = plot_patches(
                patch_list=[all_s2_r[i, ...]] + models_errs,
                title_list=[
                    f"Sentinel {info[0]} \n"
                    f"{info[1][:4]}/{info[1][4:6]}/{info[1][6:]} - {info[2]}"
                ]
            ...
            if res_dir is not None:
                fig.savefig(
                    os.path.join(res_dir_i, f"{i}_{info[1]}_{info[2]}_patch_err.png")
                )
            lai_patch_tensors = [
                model_info["prosail_vars"][i, 6, ...].unsqueeze(0)
                for _, model_info in model_dict.items()
            ] + [all_snap_lai[i, ...]]
            ...
            plt.close("all")
    lai_scatter_dict = {}
    lai_scatter_dict["SNAP's Biophysical Processor"] = all_snap_lai.squeeze(1).reshape(
        -1
    )
    global_lim = [
        lai_scatter_dict["SNAP's Biophysical Processor"].min().item(),
    ...
    for method in ["simple_interpolate"]:
        for variable in ["lai", "lai_eff"]:
            ...
            for i, (model_name, model_info) in enumerate(model_dict.items()):
                ...
                _, _, r2_cyclical, rmse_cyclical = regression_metrics(
                    model_info["cyclical_ref_lai"].detach().cpu().numpy(),
                    model_info["cyclical_lai"].detach().cpu().numpy(),
                )
            ...
            if res_dir is not None:
                fig_picp_c.savefig(
                    os.path.join(
                        res_dir, f"picp_vs_cyclical_rmse_{method}_{variable}.png"
                    )
                )
                fig_picp_l.savefig(
                    os.path.join(res_dir, f"picp_vs_rec_loss_{method}_{variable}.png")
                )
                fig_rmse_c.savefig(
                    os.path.join(
                        res_dir, f"rmse_vs_cyclical_rmse_{method}_{variable}.png"
                    )
                )
                fig_rmse_l.savefig(
                    os.path.join(res_dir, f"rmse_vs_rec_loss_{method}_{variable}.png")
                )
    for _, model_info in model_dict.items():
        err_boxplot_dict[model_info["plot_name"]] = (
            model_info["reconstruction"][:, :10, ...] - all_s2_r[:, :10, ...]
        )
        err_scatter_dict[model_info["plot_name"]] = (
            err_boxplot_dict[model_info["plot_name"]].pow(2).mean(1).sqrt().reshape(-1)
        )

        global_lim[0] = min(
            global_lim[0], err_scatter_dict[model_info["plot_name"]].min().item()
        )
        global_lim[1] = max(
            global_lim[1], err_scatter_dict[model_info["plot_name"]].max().item()
        )
    ...
    fig, axs = plt.subplots(
        1,
        len(err_scatter_dict),
        figsize=(3 * len(err_scatter_dict), 3),
        dpi=200,
        tight_layout=True,
    )
    ...
    for i, (_, model_info) in enumerate(model_dict.items()):
        axs[i].hist(
            err_scatter_dict[model_info["plot_name"]], bins=200, range=global_lim
        )
    ...
    for i, (_, model_info) in enumerate(model_dict.items()):
        regression_plot(
            pd.DataFrame(
                {
                    "Simulated LAI": model_info["cyclical_ref_lai"]
                    .detach()
                    .cpu()
                    .numpy(),
    ...
    fig, axs = plt.subplots(
        1, len(err_scatter_dict), figsize=(3 * len(err_scatter_dict), 3), dpi=200
    )
    if len(err_scatter_dict) == 1:
        axs = [axs]
    for i, (_, model_info) in enumerate(model_dict.items()):
        axs[i].hist(
            err_boxplot_dict[model_info["plot_name"]][:, i, ...].reshape(-1),
            bins=200,
            range=global_lim,
        )
        axs[i].plot(global_lim, global_lim, "k--")
...
def compare_snap_versions_on_weiss_data(res_dir):
    def load_refl_angles(path_to_data_dir: str):
        ...
        return (
            ...
            psi,
            lai,
        )  # Warning, inverted tto and tts w.r.t my prosil version
    ...
    s2_r, s2_a, lai = load_weiss_dataset(
        os.path.join(prosailvae.__path__[0], os.pardir) + "/field_data/lai/"
    )
    s2_data = torch.from_numpy(
        np.concatenate((s2_r, np.cos(np.deg2rad(s2_a))), 1)
    ).float()
    snap_ver = ["2.1", "3A", "3B"]
...
def get_frm4veg_validation_metrics(
    model_dict, frm4veg_data_dir, filenames, method, mode, get_error=True
):
    frm4veg_results = {}
    if isinstance(filenames, str):
        for _, (model_name, model_info) in enumerate(tqdm(model_dict.items())):
            frm4veg_results[model_name] = get_frm4veg_results_at_date(
                model_info["model"],
                frm4veg_data_dir,
                BARRAX_2021_FILENAME,
                is_BVNET=False,
                get_reconstruction=get_error,
            )
        frm4veg_results["SNAP"] = get_frm4veg_results_at_date(
            model_info["model"],
            frm4veg_data_dir,
            BARRAX_2021_FILENAME,
            is_BVNET=True,
        ...
        for _, (model_name, model_info) in enumerate(tqdm(model_dict.items())):
            frm4veg_results[model_name] = interpolate_frm4veg_pred(
                model_info["model"],
                frm4veg_data_dir,
                filenames[0],
                filenames[1],
        ...
        frm4veg_results["SNAP"] = interpolate_frm4veg_pred(
            model_info["model"],
            frm4veg_data_dir,
            filenames[0],
            filenames[1],
...
def get_belsar_x_frm4veg_lai_validation_results(
    model_dict,
    belsar_results,
    barrax_results,
    barrax_2021_results,
    ...
    results = {}
    for _, (model_name, model_info) in enumerate(tqdm(model_dict.items())):
        results[model_name] = get_belsar_x_frm4veg_lai_results(
            belsar_results[model_name],
    ...
    results["SNAP"] = get_belsar_x_frm4veg_lai_results(
        belsar_results["SNAP"],
        barrax_results["SNAP"],
...
def get_frm4veg_ccc_validation_results(
    model_dict,
    barrax_results,
    barrax_2021_results,
    wytham_results,
    ...
    results = {}
    for _, (model_name, model_info) in enumerate(tqdm(model_dict.items())):
        results[model_name] = get_frm4veg_ccc_results(
            barrax_results[model_name],
            barrax_2021_results[model_name],
            wytham_results[model_name],
    ...
    results["SNAP"] = get_frm4veg_ccc_results(
        barrax_results["SNAP"],
        barrax_2021_results["SNAP"],
        wytham_results["SNAP"],
        frm4veg_ccc=frm4veg_ccc,
...
def compare_validation_regressions(
    model_dict,
    belsar_dir,
    frm4veg_data_dir,
    ...
    belsar_results = get_belsar_validation_results(
        model_dict, belsar_dir, res_dir, method=method, mode=mode, get_error=get_error
    )
    ...
    barrax_results = get_frm4veg_validation_metrics(
        model_dict, frm4veg_data_dir, BARRAX_FILENAMES, method=method, mode=mode
    )
    barrax_2021_results = get_frm4veg_validation_metrics(
        model_dict,
        frm4veg_2021_data_dir,
        BARRAX_2021_FILENAME,
        method=method,
    ...
    wytham_results = get_frm4veg_validation_metrics(
        model_dict, frm4veg_data_dir, WYTHAM_FILENAMES, method=method, mode=mode
    )
    ...
    global_results = pd.concat(
        (global_results, pd.DataFrame({"model": ["SNAP"]})), axis=0
    ...
    for variable in ["lai_eff", "lai"]:
        ...
        validation_lai_results[variable] = get_belsar_x_frm4veg_lai_validation_results(
            model_dict,
            belsar_results,
            barrax_results,
            barrax_2021_results,
        ...
        for model, df_results in validation_lai_results[variable].items():
            df_results.to_csv(
                os.path.join(res_dir, f"{mode}_{method}_{variable}_{model}.csv")
            )
            rmse, picp, mpiw, mestdr = get_validation_global_metrics(
                df_results, decompose_along_columns=["Campaign"], variable=variable
            )
        ...
        for key, values in rmse["Campaign"].items():
            ...
            for model, df_results in validation_lai_results[variable].items():
                all_rmse.append(
                    lai_rmse_dict[variable][model]["Campaign"][key].values.reshape(-1)[
                        0
                    ]
                )
        ...
        for key, values in picp["Campaign"].items():
            ...
            for model, df_results in validation_lai_results[variable].items():
                all_picp.append(
                    lai_picp_dict[variable][model]["Campaign"][key].values.reshape(-1)[
                        0
                    ]
                )
        ...
        for key, values in mpiw["Campaign"].items():
            ...
            for model, df_results in validation_lai_results[variable].items():
                all_mpiw.append(
                    lai_mpiw_dict[variable][model]["Campaign"][key].values.reshape(-1)[
                        0
                    ]
                )
        ...
        for key, values in mestdr["Campaign"].items():
            ...
            for model, df_results in validation_lai_results[variable].items():
                all_mestdr.append(
                    lai_mestdr_dict[variable][model]["Campaign"][key].values.reshape(
                        -1
                    )[0]
                )
            mestdr_dict[key] = all_mestdr
        pd.DataFrame(data=rmse_dict).to_csv(
            os.path.join(res_dir, f"{mode}_{method}_{variable}_campaign_rmse.csv")
        )
        pd.DataFrame(data=picp_dict).to_csv(
            os.path.join(res_dir, f"{mode}_{method}_{variable}_campaign_picp.csv")
        )
        pd.DataFrame(data=mpiw_dict).to_csv(
            os.path.join(res_dir, f"{mode}_{method}_{variable}_campaign_mpiw.csv")
        )
        pd.DataFrame(data=mestdr_dict).to_csv(
            os.path.join(res_dir, f"{mode}_{method}_{variable}_campaign_mestdr.csv")
        )
        global_results = pd.concat(
            (
                global_results,
                ...
                pd.DataFrame(data=mestdr_dict),
            ),
            axis=1,
        )
        if not len(model_dict) > 6:
            plot_lai_validation_comparison(
                model_dict,
                validation_lai_results[variable],
                res_dir=res_dir,
                prefix=f"{mode}_{method}_{variable}",
                margin=0.02,
                hue_perfs=True,
                variable=variable,
            )
            ...
            plot_lai_validation_comparison(
                ...
                validation_lai_results[variable],
                res_dir=res_dir,
                prefix=f"{mode}_{method}_{variable}_Land_cover",
                margin=0.02,
                hue="Land cover",
                variable=variable,
            )
            ...
            plot_lai_validation_comparison(
                ...
                validation_lai_results[variable],
                res_dir=res_dir,
                prefix=f"{mode}_{method}_{variable}_Time_delta",
                margin=0.02,
                hue="Time delta",
                variable=variable,
            )
            ...
            plot_lai_validation_comparison(
                ...
                validation_lai_results[variable],
                res_dir=res_dir,
                prefix=f"{mode}_{method}_{variable}_Campaign",
                margin=0.02,
                hue="Campaign",
                legend_col=2,
                hue_perfs=True,
        ...
        np.save(
            os.path.join(res_dir, f"{mode}_{method}_{variable}_Campaign_rmse.npy"), rmse
        )
        np.save(
            os.path.join(res_dir, f"{mode}_{method}_{variable}_Campaign_picp.npy"), picp
        )
        pd.DataFrame(data={"rmse": all_rmse}).to_csv(
            os.path.join(res_dir, f"{mode}_{method}_{variable}_rmse_all.csv")
        )
        pd.DataFrame(data={"picp": all_picp}).to_csv(
            os.path.join(res_dir, f"{mode}_{method}_{variable}_picp_all.csv")
        )
        pd.DataFrame(data={"mpiw": all_mpiw}).to_csv(
            os.path.join(res_dir, f"{mode}_{method}_{variable}_mpiw_all.csv")
        )
        pd.DataFrame(data={"mestdr": all_mestdr}).to_csv(
            os.path.join(res_dir, f"{mode}_{method}_{variable}_mestdr_all.csv")
        )
    ...
    for variable in ["ccc_eff", "ccc"]:
        ...
        validation_lai_results[variable] = get_frm4veg_ccc_validation_results(
            model_dict,
            barrax_results,
            barrax_2021_results,
            wytham_results=wytham_results,
        ...
        for key, values in rmse["Campaign"].items():
            ...
            for model, df_results in validation_lai_results[variable].items():
                all_rmse.append(
                    ccc_rmse_dict[variable][model]["Campaign"][key].values.reshape(-1)[
                        0
                    ]
                )
            rmse_dict[key] = all_rmse
        for key, values in picp["Campaign"].items():
            ...
            for model, df_results in validation_lai_results[variable].items():
                all_picp.append(
                    ccc_picp_dict[variable][model]["Campaign"][key].values.reshape(-1)[
                        0
                    ]
                )
            picp_dict[key] = all_picp
        for key, values in mpiw["Campaign"].items():
            ...
            for model, df_results in validation_lai_results[variable].items():
                all_mpiw.append(
                    ccc_mpiw_dict[variable][model]["Campaign"][key].values.reshape(-1)[
                        0
                    ]
                )
            mpiw_dict[key] = all_mpiw
        for key, values in mestdr["Campaign"].items():
            ...
            for model, df_results in validation_lai_results[variable].items():
                all_mestdr.append(
                    ccc_mestdr_dict[variable][model]["Campaign"][key].values.reshape(
                        -1
                    )[0]
                )
            mestdr_dict[key] = all_mestdr
...
def get_models_validation_rec_loss(model_dict, loader):
    ...
    for model_name, model_info in model_dict.items():
        if model_info["supervised"]:
            ...
            loss_dict = model_info["model"].validate(
                loader, n_samples=10 if not socket.gethostname() == "CELL200973" else 2
            )
...
def get_models_validation_cyclical_rmse(model_dict, loader, lai_precomputed=False):
    ...
    with torch.no_grad():
        for _, model_info in model_dict.items():
            model_info["cyclical_rmse"] = (
                model_info["model"]
                .get_cyclical_rmse_from_loader(loader, lai_precomputed=lai_precomputed)
                .detach()
                .cpu()
                .item()
            )
...
def main():
    ...
    if socket.gethostname() == "CELL200973":
        args = [
            "-m",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/config/model_dict_dev.json",
            "-d",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/patches/",
            "-r",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/results/comparaison/",
            "-sp",
            "True",
        ]
        ...
        projected_data_dir = (
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/projected_data"
        )
    else:
    ...
    model_dict, test_loader, valid_loader, info_test_data = get_model_and_dataloader(
        parser
    )
    if parser.save_projected:
        save_common_cyclical_dataset(
            model_dict, valid_loader, projected_data_dir, max_batches=50
        )
    common_cyclical_loader = load_cyclical_data_set(projected_data_dir, batch_size=1)
    common_cyclical_rmse = get_models_validation_cyclical_rmse(
        model_dict, common_cyclical_loader, lai_precomputed=True
    )
    pd.DataFrame(
        data={"model": model_dict.keys(), "common_cyclical_rmse": common_cyclical_rmse}
    ).to_csv(os.path.join(res_dir, "common_cyclical_rmse.csv"))

    cyclical_loader = valid_loader
    losses = get_models_validation_rec_loss(model_dict, valid_loader)
    pd.DataFrame(data={"model": model_dict.keys(), "loss": losses}).to_csv(
        os.path.join(res_dir, "loss.csv")
    )
    cyclical_rmse = get_models_validation_cyclical_rmse(model_dict, cyclical_loader)
    pd.DataFrame(
        data={"model": model_dict.keys(), "cyclical_rmse": cyclical_rmse}
    ).to_csv(os.path.join(res_dir, "self_cyclical_rmse.csv"))
    mode = "sim_tg_mean"
    method = "simple_interpolate"
    ...
    (
        ...
        ccc_mestdr_dict,
        global_results,
    ) = compare_validation_regressions(
        model_dict,
        belsar_dir,
        frm4veg_data_dir,
        frm4veg_2021_data_dir,
        ...
        (
            ...
            pd.DataFrame(
                data={
                    ...
                    "common_cyclical_rmse": common_cyclical_rmse,
                }
            ),
        ),
        axis=1,
    ...
    (model_dict, all_s2_r, all_snap_lai, all_snap_cab, all_snap_cw) = get_model_results(
        model_dict,
        test_loader,
        info_test_data,
