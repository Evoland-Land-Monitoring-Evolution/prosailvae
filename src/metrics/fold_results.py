...
def get_results_dirs_names():
    ...
    if len(parser.root_results_dir) == 0:
        root_res_dir = os.path.join(
            os.path.join(os.path.dirname(prosailvae.__file__), os.pardir),
            "results/37099873_jobarray/",
        )
    else:
...
def main():
    ...
    for i, dir_name in enumerate(res_dirs):
        ...
        all_picp[i, :, :] = torch.from_numpy(
            pd.read_csv(dir_name + "/metrics/picp.csv", index_col=[0])
            .drop(columns=["alpha"])
            .values
        )
        all_mpiw[i, :, :] = torch.from_numpy(
            pd.read_csv(dir_name + "/metrics/mpiw.csv", index_col=[0])
            .drop(columns=["alpha"])
            .values
        )
        all_mpiwr[i, :, :] = torch.from_numpy(
            pd.read_csv(dir_name + "/metrics/mpiwr.csv", index_col=[0]).values
        )
        all_mae[i, :] = torch.from_numpy(
            pd.read_csv(dir_name + "/metrics/mae.csv", index_col=[0]).values
        ).squeeze()
        all_maer[i, :] = torch.from_numpy(
            pd.read_csv(dir_name + "/metrics/maer.csv", index_col=[0]).values
        ).squeeze()
        all_lat_nll[i, :] = torch.load(
            dir_name + "/params_nll.pt", map_location=torch.device("cpu")
        ).squeeze()
        all_aer_percentiles[i, :, :] = torch.load(
            dir_name + "/metrics/aer_percentiles.pt", map_location=torch.device("cpu")
        )
        all_are_percentiles[i, :, :] = torch.load(
            dir_name + "/metrics/are_percentiles.pt", map_location=torch.device("cpu")
        )
        pass
    pd.DataFrame(
        data=all_test_losses.reshape(1, -1).cpu().numpy(), columns=model_names
    ).to_csv(gathered_res_dir + "/all_losses.csv")
