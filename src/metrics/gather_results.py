...
import prosailvae
from prosailvae.ProsailSimus import BANDS, PROSAILVARS, ProsailVarsDist
...
def main():
    ...
    for i, dir_name in enumerate(res_dirs):
        ...
        pass
    plot_metric_boxplot(
        aer_percentiles, gathered_res_dir, "agregated_aer", model_names=model_names
    )
    plot_metric_boxplot(
        are_percentiles, gathered_res_dir, "agregated_are", model_names=model_names
    )
    plot_losses(val_losses, gathered_res_dir, model_names=model_names)
    pd.DataFrame(data=np.array(val_losses).reshape(1, -1), columns=model_names).to_csv(
