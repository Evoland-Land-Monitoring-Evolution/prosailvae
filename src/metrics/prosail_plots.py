...
from prosailvae.prosail_var_dists import get_prosail_var_bounds, get_prosail_var_dist
from prosailvae.ProsailSimus import BANDS, PROSAILVARS
...
def plot_single_lat_hist_2D(
    ...
    if heatmap is None or extent is None:
        if tgt_dist is not None and sim_pdf is not None and sim_support is not None:
            ...
            raise ValueError(
                "Please input either heatmap and extent, or tgt_dist, sim_pdf and sim_support"
            )
...
def customized_box_plot(percentiles_tensor, axes, redraw=True, *args, **kwargs):
    ...
    for box_no in range(n_box):
        ...
        if len(pdata) == 6:
            ...
            raise ValueError(
                "Percentile arrays for customized_box_plot must have either 5 or 6 values"
            )
...
def frm4veg_plots(lai_pred, ccc_pred, data_dir, filename, s2_r=None, res_dir=None):
    ...
    lai_pred_at_site = lai_pred[
        :, gdf_lai["y_idx"].values.astype(int), gdf_lai["x_idx"].values.astype(int)
    ].reshape(-1)
    ...
    if hue is None:
        ...
        if hue2 is None:
            ...
            if legend_col > 0:
                ...
                handles = (
                    [title_proxy] + [f("s", color) for color in colors] + [title_proxy]
                )
