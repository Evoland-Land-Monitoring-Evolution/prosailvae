...
def save_results(
    ...
    if all_train_loss_df is not None:
        ...
        if plot_results:
            loss_curve(
                all_train_loss_df, save_file=loss_dir + "train_loss.svg", log_scale=True
            )
    if all_valid_loss_df is not None:
        all_valid_loss_df.to_csv(loss_dir + "valid_loss.csv")
        if plot_results:
            loss_curve(
                all_valid_loss_df, save_file=loss_dir + "valid_loss.svg", log_scale=True
            )
    ...
    if plot_results:
        ...
        normed_rec_dist = (
            rec_dist.to(device) - ssimulator.norm_mean.to(device)
        ) / ssimulator.norm_std.to(device)
        normed_refl_dist = (
            refl_dist.to(device) - ssimulator.norm_mean.to(device)
        ) / ssimulator.norm_std.to(device)
    ...
    if params["k_fold"] > 1:
        ...
        if len(same_fold_dir) > 0:
            if overwrite_xp:
                ...
                raise ValueError(
                    f"The same experiment (fold) has already been carried out at {same_fold_dir}.\n Please change the number of fold or allow overwrite"
                )
    else:
...
def setupResults(res_dir):
    ...
    logging.basicConfig(
        filename=res_dir + "/results_log.log", level=logging.INFO, force=True
    )
...
def configureEmissionTracker(parser):
    ...
    try:
        ...
        tracker = OfflineEmissionsTracker(
            ...
            output_dir=os.path.join(parser.root_results_dir, os.pardir),
        )
        tracker.start()
...
def main():
    if socket.gethostname() == "CELL200973":
        args = [
            ...
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/",
            "-t",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/real_data/torchfiles/",
        ]
    ...
    if len(parser.root_results_dir) == 0:
        res_dir = os.path.join(
            os.path.join(os.path.dirname(prosailvae.__file__), os.pardir), "results/"
        )
    else:
