...
from prosailvae.dist_utils import (
    kl_tntn,
    numerical_kl_from_pdf,
    sample_truncated_gaussian,
    truncated_gaussian_cdf,
...
def get_parser():
    ...
    parser.add_argument(
        "-r", dest="results_dir", help="path to results directory", type=str, default=""
    )

    parser.add_argument(
        "-e", dest="epochs", help="number of epochs", type=int, default=1000
    )

    parser.add_argument("-n", dest="n_model_train", type=int, default=20)

    parser.add_argument("-i", dest="init_models", type=bool, default=False)

    parser.add_argument("-lr", dest="lr", type=float, default=0.001)

    parser.add_argument("-f", dest="fold_xp", type=bool, default=False)

    parser.add_argument("-t", dest="third_layer", type=bool, default=False)
    return parser
...
def sample_from_dist(
    ...
    n: int = 100,
    kernel_p: Callable | None = None,
    kernel_q: Callable | None = None,
):
    """
...
def swap_sampling(
    samples: torch.Tensor, tgt_dist_samples: torch.Tensor, allow_doubles: bool = True
):
    """
...
def swap_sampling_truncated_gaussians(
    ...
    for _ in range(n_samples):
        ...
        while idx in samples_idx:
            tgt_sample = sample_truncated_gaussian(
                mu.reshape(1, 1),
                sigma.reshape(1, 1),
                n_samples=1,
...
def divergence_contour_tg(
    ...
    cdf_1 = truncated_gaussian_cdf(
        torch.arange(low, high, dx),
        torch.as_tensor(mu0),
        torch.as_tensor(sigma0),
        lower=torch.as_tensor(low),
        upper=torch.as_tensor(high),
    )
    pdf_1 = truncated_gaussian_pdf(
        torch.arange(low, high, dx),
        torch.as_tensor(mu0),
        torch.as_tensor(sigma0),
        lower=torch.as_tensor(low),
    ...
    ax.set_title(
        f"{divergence} {metric_title} between truncated gaussians with $p_1 = TG({mu0}, {sigma0})$"
    )
    ax.scatter([mu0], [sigma0], c="r")
    return fig, ax


def simulate_data_sets(
    n_eval: int = 20000,
...
def prepare_datasets(
    ...
    with torch.no_grad():
        ...
        snap_lai = snap_nn.forward(
            torch.cat((s2_r, np.cos(np.deg2rad(s2_a))), 1).to(snap_nn.device)
        ).cpu()
    ...
    data_s2 = torch.cat(
        (s2_r, np.cos(np.deg2rad(s2_a)), snap_lai), 1
    )  # S2 reflectances and LAI produced by SNAP

    s2_r, prosail_vars = load_weiss_dataset(
    ...
    data_weiss = torch.from_numpy(
        np.concatenate((s2_r, np.cos(np.deg2rad(s2_a)), lai.reshape(-1, 1)), 1)
    )
    seed = 4567895683301
    ...
    for i, mu in enumerate(tg_mu):
        ...
        for j, sigma in enumerate(tg_sigma):
            ...
            #                             allow_doubles=False)
            idx_samples = swap_sampling_truncated_gaussians(
                data_train[:, -1],
                mu.reshape(1, 1),
                sigma.reshape(1, 1),
            ...
            h, p = np.histogram(
                data_train[idx_samples, -1].squeeze(),
                bins=200,
                range=[0, 14],
                density=True,
            )
            cum_hist = np.cumsum(h) * np.diff(p)[0]
            cdf_hist = truncated_gaussian_cdf(
                torch.from_numpy(p[:-1]),
                mu,
                sigma,
                lower=torch.tensor(0),
                upper=torch.tensor(14),
            ...
            print(
                f"number of samples for mu = {mu.item()} sigma = {sigma.item()} (kl = {kl:.2e}, "
            ...
            if plot_dist:
                ...
                ax.plot(
                    ...
                    "r",
                    label=f"sampling distribution (kl={kl:.2f})",
                )
                ax.set_xlabel("LAI")
                ax.set_xlim([0, 14])
                ax.legend()
                fig.savefig(
                    save_dir + f"/samples_lai_mu_{mu.item()}_sigma_{sigma.item()}.png"
                )
                ...
                fig.savefig(
                    save_dir
                    + f"/cdf_samples_lai_mu_{mu.item()}_sigma_{sigma.item()}.png"
                )
                plt.close("all")
    ...
    for i, mu in enumerate(tg_mu):
        for j, sigma in enumerate(tg_sigma):
            ...
            #                                           dx=bin_edges[0]))
            torch.save(
                data_train[idx_samples, :],
                save_dir + f"/tg_lai_mu_{mu.item()}_sigma_{sigma.item()}_data.pth",
            )
...
def get_model_metrics(test_data, model, all_valid_losses=[]):
    ...
    with torch.no_grad():
        ...
        reg_m, reg_b = np.polyfit(
            lai_true.squeeze().numpy(), lai_pred.squeeze().numpy(), 1
        )
...
def get_n_model_metrics(
    train_loader,
    valid_loader,
    test_loader_list: list | None = None,
    n_models: int = 10,
    epochs: int = 500,
    lr: float = 0.001,
    ...
    for i in range(n_models):
        ...
        lr_scheduler = ReduceLROnPlateau(
            optimizer=optimizer, patience=patience, threshold=0.001
        )
        _, all_valid_losses, _ = snap_nn.train_model(
            train_loader,
            valid_loader,
            optimizer,
            epochs=epochs,
        ...
        for j, test_loader in enumerate(test_loader_list):
            with torch.no_grad():
                ...
                metrics[i, j, :] = get_model_metrics(
                    test_data, snap_nn, all_valid_losses=all_valid_losses
                )
    return metrics
...
def get_pixel_log_likelihood_with_weiss(s2_r, lai, n_components=128, max_iter=500):
    ...
    s2_r_ref, _, lai_ref = load_weiss_dataset(
        os.path.join(prosailvae.__path__[0], os.pardir) + "/field_data/lai/",
        mode="snap",
    )
    ...
    lai_std = np.std(lai)
    gm = GaussianMixture(
        n_components=n_components, random_state=0, max_iter=max_iter, verbose=1
    ).fit(
        np.concatenate(
            (
                (s2_r_ref - s2_ref_mean) / s2_ref_std,
                (lai_ref.reshape(-1, 1) - lai_mean) / lai_std,
            ),
            1,
        )
    )
    return gm.score_samples(
        np.concatenate(
            (
                (s2_r - s2_ref_mean) / s2_ref_std,
                (lai.reshape(-1, 1) - lai_mean) / lai_std,
            ),
            1,
        )
    )
...
def get_boxplot_symlog_width(
    positions: np.ndarray, threshold: float = 0.01, linear_width: float = 0.1
):
    symlog_width = np.zeros_like(positions).astype(float)
    symlog_width[np.where(positions <= threshold)] = threshold * linear_width
    symlog_width[np.where(positions > threshold)] = 10 ** (
        np.log10(positions[positions > threshold]) + linear_width / 2.0
    ) - 10 ** (np.log10(positions[positions > threshold]) - linear_width / 2.0)
...
def weiss_dataset_lai_vs_ll(res_dir):
    ...
    train_loader, valid_loader = get_loaders(
        data_weiss, seed=86294692001, valid_ratio=0.1, batch_size=256
    )
    ver = "3A"
    snap_nn = SnapNN(
        device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), ver=ver
    )
    lr = 0.001
    patience = 10
    epochs = 1000
    ...
    loader = valid_loader
    loader_name = "valid"
    loader_pixels_ll = get_pixel_log_likelihood_with_weiss(
        loader.dataset[:][0][:, :8].numpy(),
    ...
    with torch.no_grad():
        absolute_errors = (
            (
                snap_nn.forward(loader.dataset[:][0].to(snap_nn.device))
                - loader.dataset[:][1].to(snap_nn.device)
            )
            .abs()
            .squeeze()
            .cpu()
    ...
    sc = ax.scatter(
        loader_pixels_ll,
        absolute_errors,
        c=loader.dataset[:][1].squeeze().numpy(),
        s=0.5,
    )
    plt.colorbar(sc)
    ...
    fig.savefig(
        res_dir + f"/scatter_abs_error_vs_loglikelihood_weiss_{loader_name}.png"
    )
    fig, ax = plt.subplots()
    ...
    for j in range(mean_metrics.size(1)):  # dataset
        for k in range(mean_metrics.size(2)):  # Métrique
            ...
            if metrics_ref[:, j, k].min() == metrics_ref[:, j, k].max():
                ax.axvline(
                    metrics_ref[:, j, k].min(), c="k", label=f"SNAP {metrics_names[k]}"
                )
            else:
                ax.hist(
                    metrics_ref[:, j, k],
                    label=f"SNAP {metrics_names[k]}",
                    bins=50,
                    color="k",
                )
            ax.legend()
            ...
            fig.savefig(
                res_dir
                + f"/{eval_data_name[j]}_{metrics_names[k]}_histogram_vs_{file_suffix}.png"
            )
            ...
            fig.savefig(
                res_dir
                + f"/means_{eval_data_name[j]}_{metrics_names[k]}_vs_{file_suffix}.png"
            )
            ...
            fig.savefig(
                res_dir
                + f"/median_{eval_data_name[j]}_{metrics_names[k]}_vs_{file_suffix}.png"
            )
            ...
            if xlog_scale:
                linthresh = 0.01
                widths = get_boxplot_symlog_width(
                    positions=x_positions, threshold=linthresh, linear_width=0.1
                )
                ax.boxplot(
                    all_metrics[:, :, j, k],
            ...
            fig.savefig(
                res_dir
                + f"/boxplot_{eval_data_name[j]}_{metrics_names[k]}_vs_{file_suffix}.png"
            )

            # scatterplot_on metrics
            fig, ax = plt.subplots(1, dpi=150, tight_layout=True)
            ax.scatter(
                x_positions.repeat(all_metrics.size(1), 1).transpose(1, 0).reshape(-1),
                all_metrics[:, :, j, k].reshape(-1),
                s=0.5,
            )
            if metrics_ref[:, j, k].min() == metrics_ref[:, j, k].max():
            ...
            fig.savefig(
                res_dir
                + f"/scatter_{eval_data_name[j]}_{metrics_names[k]}_vs_{file_suffix}.png"
            )
            plt.close("all")
...
def ll_plots(res_dir, test_loader, snap_ref, eval_data_name, k):
    ...
    with torch.no_grad():
        absolute_errors = (
            (
                snap_ref.forward(test_loader.dataset[:][0].to(snap_ref.device))
                - test_loader.dataset[:][1].to(snap_ref.device)
            )
            .abs()
            .squeeze()
            .cpu()
    ...
    sc = ax.scatter(
        loader_pixels_ll,
        absolute_errors,
        c=test_loader.dataset[:][1].squeeze().numpy(),
        s=0.5,
    )
    plt.colorbar(sc)
    # ax.set_xscale('symlog')
    ...
    ax.set_ylabel("LAI Absolute error")
    fig.savefig(
        res_dir + f"/scatter_abs_error_vs_loglikelihood_{eval_data_name[k]}.png"
    )
    fig, ax = plt.subplots()
    sc = ax.scatter(
        test_loader.dataset[:][1].squeeze().numpy(), loader_pixels_ll, s=0.5
    )
    # ax.set_xscale('symlog')
    ax.set_ylabel("Reflectances log-likelihood from the simulated dataset distribution")
    ax.set_xlabel("LAI")
    fig, ax = plt.subplots()
    sc = ax.hist2d(
        test_loader.dataset[:][1].squeeze().numpy(), loader_pixels_ll, bins=100
    )
    # ax.set_xscale('symlog')
    ax.set_ylabel("Reflectances log-likelihood from the simulated dataset distribution")
    ax.set_xlabel("LAI")
...
def get_metrics_ref(data_eval_list, train_data_list, ver="3A"):
    ...
    for k, data_eval in enumerate(data_eval_list):
        test_loader, _ = get_loaders(
            data_eval, seed=86294692001, valid_ratio=0, batch_size=256
        )
        test_loader_list.append(test_loader)
        metrics_ref = []
        for i, train_data in enumerate(train_data_list):
            _, valid_loader = get_loaders(
                train_data, seed=86294692001, valid_ratio=0.1, batch_size=256
            )
...
def main():
    ...
    if socket.gethostname() == "CELL200973":
        args = [
            "-d",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/snap_validation_data/",
            "-r",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/results/snap_validation/",
            "-e",
            "3",
            "-n",
            "2",
    ...
    if fold_xp:
        ...
        metrics_ref, test_loader_list = get_metrics_ref(
            data_eval_list, fold_data_list, ver=ver
        )
        if compute_metrics:
            mean_metrics = []
            all_metrics = []
            for i, fold_data in enumerate(fold_data_list):
                train_loader, valid_loader = get_loaders(
                    fold_data, seed=86294692001, valid_ratio=0.1, batch_size=256
                )
                metrics = get_n_model_metrics(
                    train_loader,
                    valid_loader,
        ...
        metrics_ref, test_loader_list = get_metrics_ref(
            data_eval_list, tg_data_list, ver=ver
        )
        if compute_metrics:
            ...
            for i, tg_data in enumerate(tg_data_list):
                kl[i] = list_kl[i]
                train_loader, valid_loader = get_loaders(
                    tg_data, seed=86294692001, valid_ratio=0.1, batch_size=256
                )
                metrics = get_n_model_metrics(
                    train_loader,
                    valid_loader,
        ...
        for j in range(mean_metrics.size(1)):
            ...
            sc = ax.scatter(
                all_metrics[:, :, j, 5],
                all_metrics[:, :, j, 1],
                c=kl.repeat(all_metrics.size(1), 1).transpose(1, 0).reshape(-1),
                s=1,
            )
            plt.colorbar(
                sc,
