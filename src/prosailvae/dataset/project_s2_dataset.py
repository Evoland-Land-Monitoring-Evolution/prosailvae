...
def get_parser():
    ...
    parser.add_argument(
        "-o", dest="output_dir", help="path to ouptput directory", type=str, default=""
    )
    return parser
...
def get_model_and_dataloader(parser):
    ...
    """
    train_loader, valid_loader, test_loader = get_train_valid_test_loader_from_patches(
        parser.data_dir, bands=torch.arange(10), batch_size=1, num_workers=0
    )
...
def project_loader_patches(
    model, loader, mode="lat_mode", loader_output=False, batch_size=1, max_batches=None
):
    inferred_BV = []
    projected_images = []
    angles = []
    for i, batch in tqdm(enumerate(loader)):
        ...
        (
            ...
            batch, model, patch_size=32, bands=torch.arange(10), mode=mode, no_rec=False
        )
    ...
    if loader_output:
        return DataLoader(
            TensorDataset(
                prosail_s2_sim_refl.detach().cpu(),
                angles.detach().cpu(),
...
def project_loader_pixellic(
    model, loader, mode="lat_mode", loader_output=False, batch_size=1
):
    inferred_BV = []
    projected_images = []
    angles = []
    ...
    if loader_output:
        return DataLoader(
            TensorDataset(
                prosail_s2_sim_refl.detach().cpu(), prosail_sim_vars.detach().cpu()
            ),
            batch_size=batch_size,
        )
    else:
...
def save_common_cyclical_dataset(model_dict, loader, output_dir="", max_batches=None):
    ...
    for _, model_info in model_dict.items():
        ...
        s2_r, s2_a, sim = project_loader_patches(
            model,
            loader,
            mode="lat_mode",
            loader_output=False,
    ...
    torch.save(
        torch.cat((all_s2_r, all_s2_a, all_sim), 1),
        os.path.join(output_dir, "common_projected_data_set.pth"),
    )
...
def load_cyclical_data_set(dir, batch_size=1):
    ...
    return DataLoader(
        TensorDataset(data[:, :10, ...], data[:, 10:13, ...], data[:, 13:, ...]),
        batch_size=batch_size,
    )
...
def main():
    if socket.gethostname() == "CELL200973":
        args = [
            "-m",
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/config/model_data_generate_dict.json",
            "-d",
    ...
    (
        ...
        valid_loader,
        info_test_data,
    ) = get_model_and_dataloader(parser)
    io_coeffs = load_standardize_coeffs(model_dict["dir_path"])
    ...
    for loader, name in zip([train_loader, test_loader], ["train_", "test_"]):
        print(name)
        if pixellic:
            prosail_s2_sim_refl, angles, prosail_sim_vars = project_loader_pixellic(
                model_dict["model"], loader, mode="lat_mode"
            )
        else:
            prosail_s2_sim_refl, prosail_sim_vars = project_loader_patches(
                model_dict["model"], loader, mode="lat_mode"
            )

        torch.save(
            prosail_sim_vars,
            os.path.join(parser.output_dir, name + "prosail_sim_vars.pt"),
        )
        torch.save(
            prosail_s2_sim_refl,
            os.path.join(parser.output_dir, name + "prosail_s2_sim_refl.pt"),
        )
        torch.save(
            io_coeffs.bands.loc, os.path.join(parser.output_dir, f"{name}norm_mean.pt")
        )
        torch.save(
            io_coeffs.bands.scale, os.path.join(parser.output_dir, f"{name}norm_std.pt")
        )
        torch.save(
            io_coeffs.angles.loc,
            os.path.join(parser.output_dir, f"{name}angles_loc.pt"),
        )
        torch.save(
            io_coeffs.angles.scale,
            os.path.join(parser.output_dir, f"{name}angles_scale.pt"),
        )
        torch.save(
            io_coeffs.idx.loc, os.path.join(parser.output_dir, f"{name}idx_loc.pt")
        )
        torch.save(
            io_coeffs.idx.scale, os.path.join(parser.output_dir, f"{name}idx_scale.pt")
        )
    pass
