...
def normal_pdf(x, mu, sig, eps=1e-9):
    return torch.exp((-((x - mu) ** 2)) / (2 * sig**2 + eps)) / (
        (sig + eps) * (2 * pi) ** 0.5
    )
...
def get_normal_pdf(mu, sigma, support_sampling=0.01, interval_bound=1, eps=1e-9):
    ...
    if pdf.sum() == 0:
        pdf = torch.tensor(singular_pdf(support.detach().numpy(), mu.squeeze()))
    return pdf, support
...
def numerical_kl_tn_uniform(
    mu,
    sigma,
    support=torch.arange(0, 1, 0.001),
    eps=1e-9,
...
def truncated_gaussian_pdf(
    x, mu, sigma, eps=1e-9, lower=torch.tensor(0), upper=torch.tensor(1)
):
    return (
        normal_pdf(x, mu, sigma)
        / (
            normal_cdf(upper * torch.ones_like(mu), mu, sigma)
            - normal_cdf(lower * torch.ones_like(mu), mu, sigma)
            + torch.tensor(eps)
        )
        * (x >= lower)
        * (x <= upper)
...
def truncated_gaussian_cdf(
    x, mu, sigma, eps=1e-9, lower=torch.tensor(0), upper=torch.tensor(1)
):
    return (x > upper) + (x >= lower) * (x <= upper) * (
        normal_cdf(x, mu, sigma) - normal_cdf(lower, mu, sigma)
    ) / (
        normal_cdf(upper, mu, sigma) - normal_cdf(lower, mu, sigma) + torch.tensor(eps)
    )


def truncated_gaussian_nll(
    x, mu, sig, eps=1e-9, reduction="sum", lower=torch.tensor(0), upper=torch.tensor(1)
):
...
def get_u_bounds(mu, sigma, n_sigma=4, lower=torch.tensor(0), upper=torch.tensor(1)):
    u_ubound = truncated_gaussian_cdf(
        mu + n_sigma * sigma, mu, sigma, lower=lower, upper=upper
    )
    u_lbound = truncated_gaussian_cdf(
        mu - n_sigma * sigma, mu, sigma, lower=lower, upper=upper
    )
...
def sample_truncated_gaussian(
    mu, sigma, n_samples=1, n_sigma=4, lower=torch.tensor(0.0), upper=torch.tensor(1.0)
):
    u_ubound, u_lbound = get_u_bounds(
        mu, sigma, n_sigma=n_sigma, lower=lower, upper=upper
    )
    ...
    if n_samples == 1:
        ...
        if not len(u.squeeze().size()) == 1:
            ...
            u = u.squeeze()
    z = mu + sigma * n_dist.icdf(
        n_dist.cdf((lower * torch.ones_like(mu) - mu) / sigma)
        + u
        * (
            n_dist.cdf((upper * torch.ones_like(mu) - mu) / sigma)
            - n_dist.cdf((lower * torch.ones_like(mu) - mu) / sigma)
        )
    )
    return z
...
def get_truncated_gaussian_pdf(
    mu,
    sigma,
    support_sampling=0.001,
    eps=1e-9,
    lower=torch.tensor(0),
    upper=torch.tensor(1),
):
    ...
    return (
        truncated_gaussian_pdf(support, mu, sigma, eps=eps, lower=lower, upper=upper),
        support,
    )


def truncated_gaussians_max(
    x, mu, sigma, eps=1e-9, lower=torch.tensor(0), upper=torch.tensor(1)
):
    ...
    for i in range(len(mu)):
        max_cdf = truncated_gaussian_cdf(
            x, mu[i], sigma[i], eps=eps, lower=lower, upper=upper
        )
        cdf_prod = cdf_prod * max_cdf
        max_pdf = truncated_gaussian_pdf(
            x, mu[i], sigma[i], eps=eps, lower=lower, upper=upper
        )
...
def get_latent_ordered_truncated_pdfs(
    mu,
    sigma,
    n_sigma_interval,
    support_sampling,
    ...
    lat_pdf_support = torch.arange(
        -interval_bound, interval_bound, support_sampling
    ).to(mu.device)
    len_pdf = len(lat_pdf_support)
    batch_size = mu.size(0)
    supports = (
        lat_pdf_support.view(1, 1, -1).repeat(batch_size, latent_dim, 1).to(mu.device)
    )
    pdfs = torch.zeros((batch_size, latent_dim, len_pdf)).to(mu.device)
    pdfs_at_z = truncated_gaussian_pdf(
        supports, mu, sigma, eps=eps, lower=lower, upper=upper
    )
    cdfs_at_z = truncated_gaussian_cdf(
        supports, mu, sigma, eps=eps, lower=lower, upper=upper
    )
    for i in range(latent_dim):
        max_mat_col = max_matrix[:, i]
        max_mat_mask = max_mat_col.view(1, -1).repeat(pdfs.size(2), 1)
        selected_cdfs = torch.masked_select(
            cdfs_at_z.transpose(1, 2), max_mat_mask.bool()
        ).view(batch_size, pdfs.size(2), -1)
        prod_cdfs = torch.prod(selected_cdfs, 2, keepdim=True).view(
            pdfs.size(2), -1, batch_size
        )
...
def ordered_truncated_gaussian_nll(
    z,
    mu,
    sigma,
    max_matrix,
    ...
    for i in range(n_lat):
        pdfs_at_zi = truncated_gaussian_pdf(
            z[:, i].view(-1, 1), mu, sigma, eps=eps, lower=lower, upper=upper
        ).to(device)
        cdfs_at_zi = truncated_gaussian_cdf(
            z[:, i].view(-1, 1), mu, sigma, eps=eps, lower=lower, upper=upper
        ).to(device)
        max_mat_col = max_matrix[:, i]
        ...
        selected_cdfs = torch.masked_select(cdfs_at_zi, max_mat_mask.bool()).view(
            z.size(0), -1
        )
        selected_pdfs = torch.masked_select(pdfs_at_zi, max_mat_mask.bool()).view(
            z.size(0), -1
        )
        prod_cdfs = torch.prod(selected_cdfs, 1).view(-1, 1)
        prod_pdfs_invcdfs = selected_pdfs / (selected_cdfs + eps)
        max_pdf_lat[:, i] = (
            prod_cdfs * prod_pdfs_invcdfs.sum(axis=1).view(-1, 1)
        ).squeeze()
...
def cdf2quantile(cdf, support, alpha=[0.5]):
    distribution = scipy.interpolate.interp1d(
        cdf, support, bounds_error=False, fill_value="extrapolate"
    )
...
def cdfs2quantiles(cdfs, supports, alpha=[0.5]):
    ...
    for i in range(cdfs.size(0)):
        quantiles[i, :] = torch.from_numpy(
            cdf2quantile(cdfs[i, :], supports[i, :], alpha=alpha)
        )
    return quantiles


def convolve_pdfs(
    pdfs, supports, transfer_mat_line, n_pdf_sample_points=2001, support_max=None
):
    ...
    if transfer_mat_line.count_nonzero().item() == 1:
        ...
        convolved_pdf, convolved_pdf_support = scale_pdf(
            pdfs[:, idx_nonzero, :].view(batch_size, 1, -1),
            supports[:, idx_nonzero, :].view(batch_size, 1, -1),
            transfer_mat_line[idx_nonzero].item(),
            0,
            support_max=support_max,
        )
        convolved_pdf, convolved_pdf_support = resample_pdf(
            convolved_pdf,
        ...
        return convolved_pdf.reshape(batch_size, -1), convolved_pdf_support.reshape(
            batch_size, -1
        )
    else:
        raise NotImplementedError()
...
def resample_pdf(pdf, support, n_pdf_sample_points=1001):
    ...
    resampled_pdf = F.interpolate(
        pdf, size=n_pdf_sample_points, mode="linear", align_corners=True
    )
    resampled_support = F.interpolate(
        support, size=n_pdf_sample_points, mode="linear", align_corners=True
    )
    ...
    if len(zero_pdfs_idx) > 0:
        ...
        mode = torch.gather(
            support_zero_idx,
            2,
            pdf[zero_pdfs_idx, :, :].argmax(2).view(-1, 1, len(zero_pdfs_idx)),
        ).view(len(zero_pdfs_idx) * support.size(1), -1)
        s_pdf = singular_pdf(
            resampled_support[zero_pdfs_idx, :, :].view(
                len(zero_pdfs_idx) * support.size(1), -1
            ),
            mode,
        )
        resampled_pdf[zero_pdfs_idx, :, :] = s_pdf.view(
            len(zero_pdfs_idx), support.size(1), -1
        )
...
def scale_pdf(
    pdf, support, support_scale, support_min, support_max: float | None = None
):
    """
    ...
    if support_max is None:
        ...
        support_max = max(
            support_max, abs(ext_support[0, 0, -1]), abs(ext_support[0, 0, 0])
        )
    upper_support = (
        torch.arange(ext_support[0, 0, -1] + sampling, support_max + sampling, sampling)
        .to(pdf.device)
        .repeat(pdf.size(0), pdf.size(1), 1)
    )
    lower_support = (
        torch.arange(-support_max, ext_support[0, 0, 0], sampling)
        .to(pdf.device)
        .repeat(pdf.size(0), pdf.size(1), 1)
    )
    upper_pdf = torch.zeros((pdf.size(0), pdf.size(1), upper_support.size(2))).to(
        pdf.device
    )
    lower_pdf = torch.zeros((pdf.size(0), pdf.size(1), lower_support.size(2))).to(
        pdf.device
    )
