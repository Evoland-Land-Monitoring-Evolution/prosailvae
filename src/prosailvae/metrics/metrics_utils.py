...
from ..prosail_var_dists import get_prosail_vars_interval_width
from ..ProsailSimus import PROSAILVARS
...
def save_metrics(
    ...
    pd.DataFrame(
        data=mae.view(1, len(PROSAILVARS)).detach().cpu().numpy(),
        columns=PROSAILVARS,
        index=[0],
    ).to_csv(metrics_dir + "/mae.csv")
    df_mpwi = pd.DataFrame(
        data=mpiw.view(-1, len(PROSAILVARS)).detach().cpu().numpy(), columns=PROSAILVARS
    )
    df_mpwi["alpha"] = alpha_pi
    df_mpwi.to_csv(metrics_dir + "/mpiw.csv")
    df_picp = pd.DataFrame(
        data=picp.view(-1, len(PROSAILVARS)).detach().cpu().numpy(), columns=PROSAILVARS
    )
    df_picp["alpha"] = alpha_pi
    df_picp.to_csv(metrics_dir + "/picp.csv")

    interval_length = get_prosail_vars_interval_width(bounds_type=var_bounds_type).to(
        mpiw.device
    )
    mpiwr = (mpiw / interval_length.view(-1, 1)).transpose(0, 1)
    maer = mae / interval_length
    df_maer = pd.DataFrame(
        data=maer.view(-1, len(PROSAILVARS)).detach().cpu().numpy(), columns=PROSAILVARS
    )
    df_mpiwr = pd.DataFrame(
        data=mpiwr.view(-1, len(PROSAILVARS)).detach().cpu().numpy(),
        columns=PROSAILVARS,
    )
    ...
    aer_percentiles = (
        ae_percentiles / interval_length.view(1, -1).detach().cpu().numpy()
    )
...
def get_percentiles_from_box_plots(bp):
    ...
    for i in range(len(bp["boxes"])):
        percentiles[0, i] = torch.from_numpy(
            np.asarray(bp["caps"][2 * i].get_ydata()[0])
        )
        percentiles[1, i] = torch.from_numpy(np.asarray(bp["boxes"][i].get_ydata()[0]))
        percentiles[2, i] = torch.from_numpy(
            np.asarray(bp["medians"][i].get_ydata()[0])
        )
        percentiles[3, i] = torch.from_numpy(np.asarray(bp["boxes"][i].get_ydata()[2]))
        percentiles[4, i] = torch.from_numpy(
            np.asarray(bp["caps"][2 * i + 1].get_ydata()[0])
        )
...
def get_metrics(
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(loader, desc="Computing metrics", leave=True)):
            ...
            (
                ...
                prosail_params_mode,
                rec,
            ) = PROSAIL_VAE.point_estimate_rec(s2_r, angles, mode="sim_mode")
            lat_pdfs, lat_supports = PROSAIL_VAE.lat_space.latent_pdf(dist_params)
            sim_pdfs_i, sim_supports_i = PROSAIL_VAE.sim_space.sim_pdf(
                lat_pdfs, lat_supports, n_pdf_sample_points=n_pdf_sample_points
            )
            sim_pdfs = torch.concat([sim_pdfs, sim_pdfs_i], axis=0)
            sim_supports = torch.concat([sim_supports, sim_supports_i], axis=0)
            pheno_pi_lower = PROSAIL_VAE.sim_space.sim_quantiles(
                lat_pdfs,
                lat_supports,
                alpha=pi_lower,
                n_pdf_sample_points=n_pdf_sample_points,
            )
            pheno_pi_upper = PROSAIL_VAE.sim_space.sim_quantiles(
                lat_pdfs,
                lat_supports,
                alpha=pi_upper,
                n_pdf_sample_points=n_pdf_sample_points,
            ...
            rec_dist = torch.concat(
                [rec_dist, ssimulator.normalize(rec.squeeze(), bands_dim=1)], axis=0
            )
            rel_error_i = (prosail_params_mode.squeeze() - tgt).abs() / (
                tgt.abs() + 1e-10
            )
            rel_error = torch.concat([rel_error, rel_error_i], axis=0)
            piw_i = pheno_pi_upper - pheno_pi_lower
            piw = torch.concat([piw, piw_i], axis=0)
            pic_i = torch.logical_and(
                tgt.unsqueeze(2) > pheno_pi_lower, tgt.unsqueeze(2) < pheno_pi_upper
            ).float()
...
def get_juan_validation_metrics(
    PROSAIL_VAE,
    juan_data_dir_path,
    lai_min=0,
    dt_max=10,
    ...
    for site in sites:
        ...
        juan_dataset = TensorDataset(
            s2_r.to(PROSAIL_VAE.device),
            s2_a.to(PROSAIL_VAE.device),
            prosail_ref_params.to(PROSAIL_VAE.device),
        )
        juan_loader = DataLoader(juan_dataset, batch_size=256, num_workers=0)
        lai_nlls = PROSAIL_VAE.compute_lat_nlls(juan_loader).mean(0).squeeze()[6].cpu()
        list_lai_nlls.append(lai_nlls)
        _, _, prosail_params_mode, _ = PROSAIL_VAE.point_estimate_rec(
            s2_r.to(PROSAIL_VAE.device), s2_a.to(PROSAIL_VAE.device), mode="sim_mode"
        )
...
def get_bvnet_validation_metrics(
    PROSAIL_VAE, s2_r, s2_a, prosail_ref_params, n_pdf_sample_points=5001
):
    with torch.no_grad():
