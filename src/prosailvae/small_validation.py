...
from dataset.generate_dataset import (
    simulate_lai_with_rec_error_hist,
    simulate_lai_with_rec_error_hist_with_enveloppe,
    simulate_prosail_samples_close_to_ref,
)
from dataset.loaders import get_simloader
from dataset.preprocess_small_validation_file import (
    LAI_columns,
    get_small_validation_data,
)
...
def get_model(model_dir):
    logging.basicConfig(
        filename=model_dir + "/pv_validation.log", level=logging.INFO, force=True
    )
    ...
    if params["supervised_kl"]:
        ...
        params_sup_kl_model["sup_model_weights_path"] = (
            model_dir + "/sup_kl_model_weights.tar"
        )
    vae_file_path = model_dir + "/prosailvae_weights.tar"
    rsr_dir = "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/"
    PROSAIL_VAE = load_PROSAIL_VAE_with_supervised_kl(
        params,
        rsr_dir,
        model_dir,
        logger_name=LOGGER_NAME,
...
def compare_reflectances(s2_r, s2_r_sim, site, bands=[0]):
    ...
    bp_real = ax.boxplot(
        s2_r.transpose(0, 1),
        positions=torch.arange(0, 2 * s2_r.shape[1], 2) - dis,
        widths=0.2,
        patch_artist=True,
        showfliers=False,
    )
    bp_sim = ax.boxplot(
        s2_r_sim.transpose(0, 1),
        positions=torch.arange(0, 2 * s2_r.shape[1], 2) + dis,
        widths=0.2,
        patch_artist=True,
        showfliers=False,
    ...
    ax.legend(
        [bp_real["boxes"][0], bp_sim["boxes"][0]],
...
def lut_pred(s2_r, s2_r_sim, lai_sim):
    ...
    for i in range(s2_r.size(0)):
        s2_r_rmae = (
            ((s2_r_sim - s2_r[i, :].unsqueeze(0)) / s2_r_sim.max(0)[0].unsqueeze(0))
            .abs()
            .mean(1)
        )
...
def lut_advanced_pred(s2_r, s2_r_sim, lai_sim, s2_a, s2_a_sim):
    ...
    for i in range(s2_r.size(0)):
        ...
        s2_a_rmae = (
            (
                (close_refl_s2_a_sim - s2_a[i, :].unsqueeze(0))
                / s2_a_sim.max(0)[0].unsqueeze(0)
            )
            .abs()
            .mean(1)
        )
...
def compare_datasets():
    model_dir = (
        "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/results/best_regression_2/"
    )
    ...
    for site in ["spain1", "spain2", "italy1", "italy2", "france", "spain1"]:
        ...
        s2_r, s2_a, lais, time_delta = get_small_validation_data(
            relative_s2_time=relative_s2_time,
            site=site,
...
def plot_s2r_vs_s2_r_pred(
    ...
    if prosail_vars is not None:
        ...
        for i in range(len(PROSAILVARS)):
            title_str += f" {PROSAILVARS[i]}={prosail_vars[i]:.3f} |"
        ...
        if angles is not None:
            title_str += (
                f"\ntts={angles[0]:.1f} | tto={angles[1]:.1f} | psi={angles[2]:.1f}"
            )
        if best_mae is not None:
            title_str += f"\nmae={best_mae:.4f} |"
        if delta_t is not None:
            title_str += f" dt={delta_t:.0f}"
...
def get_quantiles_from_hist(hist, bin_range=[0, 1]):
    ...
    median = np.apply_along_axis(
        lambda a: hist_range[np.where(a < 0.50)[0][-1]], 0, cdf
    )
...
def plot_s2_sim_dist(
    s2_r_ref, aggregate_s2_hist, lai, tts, tto, psi, colors=None, lai_ref=None
):
    ...
    title = ""
    if lai_ref is not None:
        title = f"lai ref={lai_ref:.1f} | "
    title += f"lai={lai:.1f} | tts={tts:.1f} | tto={tto:.1f} | psi={psi:.1f}"
    ax.set_title(title)
...
def find_close_simulation(
    ...
    (top_n_delta, top_n_s2_r, top_n_s2_a, top_n_lais) = sort_by_smallest_deltas(
        abs_time_delta, s2_r.numpy(), s2_a.numpy(), lais.numpy(), n=n
    )
    for idx_in_situ_sample in range(len(top_n_delta)):
        ...
        if delta_t < max_delta:
            ...
            fig, ax = plot_s2r_vs_s2_r_pred(
                s2_r_ref,
                best_prosail_s2_sim,
                best_prosail_vars,
                top_n_s2_a[idx_in_situ_sample, :],
                site=site,
                lai_ref=lai_ref,
                best_mae=best_mae,
                delta_t=delta_t,
            ...
            fig.savefig(
                results_dir
                + f"/{site}_{idx_in_situ_sample}_lai_ref_{not exclude_lai}_closest_reflectance_match.svg"
            )
            fig, ax = plot_s2_sim_dist(
                s2_r_ref, aggregate_s2_hist, lai, tts, tto, psi, colors=None
            )
            fig.savefig(
                results_dir
                + f"/{site}_{idx_in_situ_sample}_lai_ref_{not exclude_lai}_simulation_distribution.svg"
            )
    pass


def plot_lai_vs_mc_error_2d_hist(
    heatmap,
    extent,
    aggregate_lai_hist,
    min_lai=0,
    ...
    fig, axs = plt.subplots(
        2, sharex=True, gridspec_kw={"height_ratios": (0.15, 0.85)}, dpi=200
    )
    ...
    axs[0].plot(
        np.linspace(min_lai, max_lai, len(aggregate_lai_hist)),
        aggregate_lai_hist.reshape(-1),
    )
    ...
    if len(all_cases_in_enveloppe_err) > 0:
        ...
        axs[1].axvline(
            ...
            c="green",
            ls="--",
            label="median LAI for simulated reflectances within uncertainty",
        )
    ...
    axs[0].set_title(
        f"Error of simulated reflectances vs LAI (Site : {site} | dt : {dt})"
    )
...
def mc_simulation_error(
    ...
    for idx_in_situ_sample in range(len(top_n_delta)):
        ...
        if delta_t <= max_delta:
            ...
            fig.savefig(
                results_dir
                + f"/{site}_{idx_in_situ_sample}_uni_{uniform_mode}_lai_vs_mc_sim_error_2d_hist.svg"
            )
...
def main():
    if socket.gethostname() == "CELL200973":
        ...
        rsr_dir = "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/data/"
        results_dir = (
            "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/results/validation/"
        )
        exclude_lai = False
        max_delta = 4
