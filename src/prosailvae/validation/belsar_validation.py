...
BELSAR_FILENAMES = [
    ...
    "2A_20180518_both_BelSAR_agriculture_database",  # Nuages mais + non
    # détectés => A
    # retirer !
    "2B_20180526_both_BelSAR_agriculture_database",
    "2A_20180528_both_BelSAR_agriculture_database",  # Nuages sur
    # l'image => A
    # retirer !
    "2A_20180620_both_BelSAR_agriculture_database",  # Nuageuse + nuages
    # non détectés => A
    # retirer !
...
all_filename_dict = {
    "2018-02-25": "2B_20180225_both_BelSAR_agriculture_database",
    ...
    "2018-05-18": "2A_20180518_both_BelSAR_agriculture_database",  # Nuages
    # mais
    # +
    # non
    # détectés
    ...
    "2018-05-28": "2A_20180528_both_BelSAR_agriculture_database",  # Nuages
    # sur
    # l'image
    # =>
    # A
    # retirer
    # !
    "2018-06-20": "2A_20180620_both_BelSAR_agriculture_database",  # Nuageuse
    # +
    # nuages
    # non
    # détectés
...
def plot_belsar_site(data_dir, filename):
    df, s2_r, s2_a, mask, xcoords, ycoords, crs = load_belsar_validation_data(
        data_dir, filename
    )
    ...
    ax.imshow(
        mask.squeeze(),
        extent=[xcoords[0], xcoords[-1], ycoords[-1], ycoords[0]],
        cmap="YlOrRd",
    )
    ...
    for xi, yi, text in zip(
        wheat_sites.centroid.x, wheat_sites.centroid.y, wheat_sites["Name"], strict=True
    ):
        ax.annotate(
            text,
            xy=(xi, yi),
    ...
    for xi, yi, text in zip(
        maize_sites.centroid.x, maize_sites.centroid.y, maize_sites["Name"], strict=True
    ):
        ax.annotate(
            text,
            xy=(xi, yi),
...
def get_prosailvae_train_parser():
    ...
    parser.add_argument(
        ...
        help="name of data files (without extension)",
        type=str,
        default="/home/uz/zerahy/scratch/prosailvae/data/silvia_validation/",
    )
...
def preprocess_belsar_validation_data(
    data_dir, filename, s2_product_name, crop="maize", margin=100
):
    output_file_name = s2_product_name[8:19] + f"_{crop}_" + filename
    data_file = filename + ".xlsx"
    if crop != "both":
        data_df = pd.read_excel(
            os.path.join(data_dir, data_file), sheet_name=f"BelSAR_{crop}", skiprows=[0]
        )
        data_df.rename(
            columns={"Date": "date", "PAI": "lai", "Sample dry weight (g)": "cm"},
            inplace=True,
        )
    else:
        wheat_df = pd.read_excel(
            os.path.join(data_dir, data_file), sheet_name="BelSAR_wheat", skiprows=[0]
        )
        wheat_df.rename(
            columns={"Date": "date", "PAI": "lai", "Sample dry weight (g)": "cm"},
            inplace=True,
        )
        maize_df = pd.read_excel(
            os.path.join(data_dir, data_file), sheet_name="BelSAR_maize", skiprows=[0]
        )
        maize_df.rename(
            columns={"Date": "date", "GAI": "lai", "Sample dry weight (g)": "cm"},
            inplace=True,
        )
    ...
    data_df = data_df.drop(
        columns=[
            ...
            "Sample fresh wieght (g)",
            "Dry matter content (%)",
            "Interline distance mean (cm)",
    ...
    (s2_r, s2_a, validity_mask, xcoords, ycoords, crs) = read_data_from_theia(
        left, bottom, right, top, src_epsg, path_to_theia_product, margin=margin
    )
...
def get_sites_geometry(data_dir, crs, crop: str | None = None):
    ...
    for layer in fiona.listlayers(os.path.join(data_dir, "BelSAR" + ".kml")):
        s = gpd.read_file(
            os.path.join(data_dir, "BelSAR" + ".kml"), driver="KML", layer=layer
        )
    ...
    sites_geometry = sites_geometry[
        sites_geometry["geometry"].apply(lambda x: x.geom_type == "Polygon")
    ]
...
def get_delta_dict(filename_dict):
    ...
    for date, filename in filename_dict.items():
        ...
        delta = (
            ...
            - datetime.strptime(filename_date_str, "%Y%m%d")
        ).days
...
closest_filename_dict = {
    "2018-05-17": "2A_20180518_both_BelSAR_agriculture_database",
...
def get_belsar_image_metrics(
    sites_geometry,
    validation_df,
    belsar_pred_dir,
    belsar_pred_filename,
    belsar_pred_file_suffix,
    date,
    delta_t,
    NO_DATA=-10000,
    get_error=True,
    ...
    for i in range(len(sites_geometry)):
        ...
        with rio.open(
            os.path.join(
                belsar_pred_dir, f"{belsar_pred_filename}{belsar_pred_file_suffix}.tif"
            ),
            mode="r",
        ) as src:
        ...
        for variable in ["lai", "cm"]:
            ...
            if not np.isnan(masked_array[pred_array_idx[variable]["mean"], ...]).all():
                d[f"{variable}_mean"] = np.nanmean(
                    masked_array[pred_array_idx[variable]["mean"], ...]
                )
                d[f"{variable}_std"] = np.nanstd(
                    masked_array[pred_array_idx[variable]["mean"], ...]
                )
        ...
        if get_error:
            if not np.isnan(masked_err).all():
                d["rec_err_mean"] = np.nanmean(masked_err)
                d["rec_err_std"] = np.nanstd(masked_err)
        ...
        if not np.isnan(masked_array[pred_array_idx[variable]["mean"], ...]).all():
            d["hspot_mean"] = np.nanmean(
                masked_array[pred_array_idx["hspot"]["mean"], ...]
            )
            d["hspot_std"] = np.nanstd(
                masked_array[pred_array_idx["hspot"]["mean"], ...]
            )
            d["hspot_sigma_mean"] = np.nanmean(
                masked_array[pred_array_idx["hspot"]["sigma"], ...]
            )
            d["hspot_sigma_std"] = np.nanstd(
                masked_array[pred_array_idx["hspot"]["sigma"], ...]
            )
...
def get_belsar_campaign_metrics_df(
    belsar_data_dir,
    filename_dict,
    belsar_pred_dir,
    file_suffix,
    ...
    for date, filename in filename_dict.items():
        validation_df, _, _, _, _, _, crs = load_belsar_validation_data(
            belsar_data_dir, filename
        )
        validation_df = validation_df[validation_df["date"] == date]
        ids = list(pd.unique(validation_df["Field ID"]).tolist())
        sites_geometry = get_sites_geometry(belsar_data_dir, crs)
        sites_geometry = sites_geometry[
            sites_geometry["Name"].apply(lambda x: x in ids)  # noqa: B023
        ]
        sites_geometry.reset_index(inplace=True, drop=True)
        delta_t = delta_dict[date]
        image_metrics = get_belsar_image_metrics(
            sites_geometry,
            validation_df,
            belsar_pred_dir,
            filename,
...
def interpolate_belsar_metrics(
    belsar_data_dir,
    belsar_pred_dir,
    method="closest",
    file_suffix="",
    ...
    if method == "simple_interpolate":
        before_metrics = get_belsar_campaign_metrics_df(
            belsar_data_dir,
            before_filename_dict,
            belsar_pred_dir,
            file_suffix,
        ...
        after_metrics = get_belsar_campaign_metrics_df(
            belsar_data_dir,
            after_filename_dict,
            belsar_pred_dir,
            file_suffix,
        ...
        for variable in [
            ...
            metrics[variable] = simple_interpolate(
                after_metrics[variable],
                before_metrics[variable],
                after_metrics["delta"],
                before_metrics["delta"],
                is_std=is_std,
            )
        metrics["delta_before"] = before_metrics["delta"]
        metrics["delta_after"] = after_metrics["delta"]
        metrics["date"] = (
            abs(metrics["delta_after"]) + abs(metrics["delta_before"])
        ) / 2
    elif method == "closest":
        metrics = get_belsar_campaign_metrics_df(
            belsar_data_dir,
            closest_filename_dict,
            belsar_pred_dir,
            file_suffix,
        ...
        for i in range(len(metrics)):
            err_before = np.abs(
                metrics["lai_mean"].iloc[i] - metrics["ref_lai"].iloc[i]
            )
            err_after = np.abs(
                after_metrics["lai_mean"].iloc[i] - after_metrics["lai_mean"].iloc[i]
            )
        ...
        for i in range(len(metrics)):
            ...
            err_after = np.abs(
                after_metrics["lai_mean"].iloc[i] - after_metrics["ref_lai"].iloc[i]
            )
            if err_after > err_before:
                metrics.iloc[i] = after_metrics.iloc[i]
                metrics["date"].iloc[i] = abs(metrics["delta"].iloc[i])
...
def save_belsar_predictions(
    ...
    for filename in tqdm(list_filenames):
        ...
        with torch.no_grad():
            (rec, sim_image, s2_r, _, sigma_image) = get_encoded_image_from_batch(
                (s2_r, s2_a),
                model,
                patch_size=32,
...
def save_bvnet_belsar_predictions(
    ...
    for filename in list_belsar_filename:
        ...
        df, s2_r_image, s2_a, mask, xcoords, ycoords, crs = load_belsar_validation_data(
            belsar_dir, filename
        )
        s2_r = torch.from_numpy(s2_r_image)[
            torch.tensor([1, 2, 3, 4, 5, 7, 8, 9]), ...
        ].float()
        mask[mask == 1.0] = np.nan
        mask[mask == 0.0] = 1.0
        if np.isnan(mask).all():
        ...
        s2_a_permutated = torch.cos(
            torch.deg2rad(
                torch.from_numpy(
                    np.concatenate((s2_a[1:2, ...], s2_a[0:1, ...], s2_a[2:, ...]), 0)
                )
            )
        ).float()
        ...
        with torch.no_grad():
            lai_pred = torch.clamp(
                lai_bvnet.forward(s2_data, spatial_mode=True).cpu(), min=0, max=None
            )
...
def get_all_belsar_predictions(
    belsar_data_dir,
    belsar_pred_dir,
    file_suffix,
    NO_DATA=-10000,
...
def main():
    ...
    list_s2_products = [
        "SENTINEL2B_20180225-105018-458_L2A_T31UFS_D.zip",
    ...
    for s2_product in list_s2_products:
        if s2_product[-4:] == ".zip":
            with zipfile.ZipFile(
                os.path.join(parser.data_dir, s2_product), "r"
            ) as zip_ref:
                zip_ref.extractall(parser.data_dir)
                s2_product = os.path.normpath(
                    str(
                        list(
                            zipfile.Path(
                                os.path.join(parser.data_dir, s2_product)
                            ).iterdir()
                        )[0]
                    )
                ).split(os.sep)[-1]
            output_file_name = preprocess_belsar_validation_data(
                parser.data_dir, parser.data_filename, s2_product, crop="both"
            )
