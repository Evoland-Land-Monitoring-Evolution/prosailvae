...
from .validation_utils import simple_interpolate, var_of_product

BARRAX_FILENAMES = [
    "2B_20180516_FRM_Veg_Barrax_20180605_V2",
    "2A_20180613_FRM_Veg_Barrax_20180605_V2",
]
BARRAX_2021_FILENAME = "2B_20210722_FRM_Veg_Barrax_20210719"
WYTHAM_FILENAMES = [
    "2A_20180629_FRM_Veg_Wytham_20180703_V2",
    "2A_20180706_FRM_Veg_Wytham_20180703_V2",
]
...
def get_data_idx_in_image(
    gdf, xmin_image_bb, ymax_image_bb, col_offset, row_offset, res=10
):
...
def get_prosailvae_train_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="FRM_Veg_Barrax_20180605",
    )
    ...
    parser.add_argument(
        ...
        help="name of data files (without extension)",
        type=str,
        default="/home/uz/zerahy/scratch/prosailvae/data/frm4veg_validation/",
    )

    parser.add_argument(
        "-p", dest="product_name", help="Theia product name", type=str, default=""
    )
    return parser
...
def get_bb_equivalent_polygon(bb, in_crs, out_crs):
    ...
    return (
        gpd.GeoDataFrame(data={"geometry": [polygon]}).set_crs(in_crs).to_crs(out_crs)
    )


def compute_frm4veg_data(
    data_dir,
    filename,
    s2_product_name,
    no_angle_data=False,
    ...
    data_df = pd.read_excel(
        os.path.join(data_dir, data_file), sheet_name="GroundData", skiprows=[0]
    )
    ...
    data_gdf = gpd.GeoDataFrame(
        ...
        geometry=gpd.points_from_xy(
            data_df["Easting Coord. "], data_df["Northing Coord. "]
        ),
    )
    ...
    if no_angle_data:
        cn_path = "/home/yoel/Documents/Dev/PROSAIL-VAE/prosailvae/"
        "data/iris_data/openEO_0_clip.nc"
        ...
        if socket.gethostname() == "CELL200973":
            ...
            (
                ...
                even_az,
                odd_az,
            ) = dataset.read_incidence_angles_as_numpy()
            joint_zen = np.array(even_zen)
            joint_zen[np.isnan(even_zen)] = odd_zen[np.isnan(even_zen)]
            del even_zen
            del odd_zen
    ...
    get_data_idx_in_image(
        data_gdf, dataset.bounds[0], dataset.bounds[3], xmin, ymin, res=10
    )

    for variable in ["lai", "lai_eff", "ccc", "ccc_eff"]:
        variable_col, uncertainty_col = get_variable_column_names(
            variable=variable, wytham=no_angle_data
        )
        gdf = (
            data_gdf[
                [
        ...
        gdf.to_file(
            os.path.join(data_dir, output_file_name + f"_{variable}.geojson"),
            driver="GeoJSON",
        )
...
def load_frm4veg_data(data_dir, filename, variable="lai"):
    gdf = gpd.read_file(
        os.path.join(data_dir, filename + f"_{variable}.geojson"), driver="GeoJSON"
    )
...
def load_iris_data(file_path, date="2018-06-29"):
    ...
    angles = torch.from_numpy(
        np.stack(
            [
                ds["sunZenithAngles"].values,
                ds["viewZenithMean"].values,
                ds["sunAzimuthAngles"].values - ds["viewAzimuthMean"].values,
            ],
            0,
        )[:, idx, ...]
    )
...
def get_frm4veg_material(frm4veg_data_dir, frm4veg_filename):
    """s2_r, s2_a, site_idx_dict, ref_dict =
    get_frm4veg_material(frm4veg_data_dir, frm4veg_filename)

    """
    site_idx_dict = {}
    ref_dict = {}
    for variable in ["lai", "lai_eff", "ccc", "ccc_eff"]:
        gdf, _, _, _, _ = load_frm4veg_data(
            frm4veg_data_dir, frm4veg_filename, variable=variable
        )
        # gdf = gdf.iloc[:51]
        ref_dict[variable] = gdf[variable].values.reshape(-1)
        ref_dict[variable + "_std"] = gdf["uncertainty"].values.reshape(-1)
        site_idx_dict[variable] = {
            "x_idx": torch.from_numpy(gdf["x_idx"].values).int(),
            "y_idx": torch.from_numpy(gdf["y_idx"].values).int(),
        }
    _, s2_r, s2_a, _, _ = load_frm4veg_data(
        frm4veg_data_dir, frm4veg_filename, variable="lai"
    )
...
def get_model_frm4veg_results(
    ...
    with torch.no_grad():
        (
            ...
            model,
            patch_size=32,
            bands=model.encoder.bands,
            mode=mode,
            padding=True,
            no_rec=not get_reconstruction,
        )
    ...
    for lai_variable in ["lai", "lai_eff"]:  # 'ccc', 'ccc_eff']:
        model_pred[lai_variable] = sim_image[
            6,
            site_idx_dict[lai_variable]["y_idx"],
            site_idx_dict[lai_variable]["x_idx"],
        ].numpy()
        model_pred[f"{lai_variable}_std"] = sigma_image[
            6,
            site_idx_dict[lai_variable]["y_idx"],
            site_idx_dict[lai_variable]["x_idx"],
        ].numpy()
        ...
        model_pred[f"{lai_variable}_rec_err"] = rec_err[
            ...,
            site_idx_dict[lai_variable]["y_idx"],
            site_idx_dict[lai_variable]["x_idx"],
        ].numpy()
        for i, band in enumerate(
            np.array(BANDS)[model.encoder.bands.detach().cpu()].tolist()
        ):
            model_pred[f"{lai_variable}_{band}_rec_err"] = band_rec_err[
                i,
                site_idx_dict[lai_variable]["y_idx"],
                site_idx_dict[lai_variable]["x_idx"],
            ].numpy()
    for ccc_variable in ["ccc", "ccc_eff"]:
        model_pred[ccc_variable] = (
            sim_image[
                1,
                site_idx_dict[ccc_variable]["y_idx"],
        ...
        for i, band in enumerate(
            ...
            model_pred[f"{ccc_variable}_{band}_rec_err"] = band_rec_err[
                i,
                site_idx_dict[ccc_variable]["y_idx"],
                site_idx_dict[ccc_variable]["x_idx"],
            ].numpy()
...
def get_bvnet_frm4veg_results(
    ...
    (bvnet_lai, bvnet_ccc, _) = get_bvnet_biophyiscal_from_batch(
        (s2_r, s2_a),
        patch_size=32,
        sensor=sensor,
        lai_bvnet=lai_bvnet,
    ...
    for variable in ["lai", "lai_eff"]:
        bvnet_results[variable] = bvnet_lai[
            ..., site_idx_dict[variable]["y_idx"], site_idx_dict[variable]["x_idx"]
        ].numpy()
        ...
        for band in BANDS:
            bvnet_results[f"{variable}_{band}_rec_err"] = np.zeros_like(
                bvnet_results[variable]
            )
    for variable in ["ccc", "ccc_eff"]:
        bvnet_results[variable] = bvnet_ccc[
            ..., site_idx_dict[variable]["y_idx"], site_idx_dict[variable]["x_idx"]
        ].numpy()
        bvnet_results[f"{variable}_std"] = np.zeros_like(bvnet_results[variable])
        bvnet_results[f"{variable}_rec_err"] = np.zeros_like(bvnet_results[variable])
...
def get_frm4veg_results_at_date(
    model,
    frm4veg_data_dir,
    filename,
    is_BVNET=False,
    ...
    (s2_r, s2_a, site_idx_dict, ref_dict) = get_frm4veg_material(
        frm4veg_data_dir, filename
    )
    if not is_BVNET:
        ...
        validation_results = get_bvnet_frm4veg_results(
            s2_r,
            s2_a,
            site_idx_dict,
            ref_dict,
    ...
    for variable in ["lai", "lai_eff", "ccc", "ccc_eff"]:
        gdf, _, _, _, _ = load_frm4veg_data(
            frm4veg_data_dir, filename, variable=variable
        )
        validation_results[f"{variable}_land_cover"] = gdf["land cover"].values
        validation_results[f"{variable}_date"] = (
            gdf["date"].apply(lambda x: (x.date() - d).days).values
        )
    return validation_results


def interpolate_frm4veg_pred(
    model,
    frm4veg_data_dir,
    filename_before,
    filename_after=None,
    ...
    validation_results_before = get_frm4veg_results_at_date(
        model,
        frm4veg_data_dir,
        filename_before,
        is_BVNET=is_BVNET,
    ...
    validation_results_after = get_frm4veg_results_at_date(
        model,
        frm4veg_data_dir,
        filename_after,
        is_BVNET=is_BVNET,
    ...
    for variable in ["lai", "lai_eff", "ccc", "ccc_eff"]:
        ...
        model_results[f"ref_{variable}_std"] = validation_results_before[
            f"ref_{variable}_std"
        ]
        gdf, _, _, _, _ = load_frm4veg_data(
            frm4veg_data_dir, filename_before, variable=variable
        )
        # gdf = gdf.iloc[:51]
        ...
        if method == "simple_interpolate":
            model_results[variable] = simple_interpolate(
                validation_results_after[variable].squeeze(),
                validation_results_before[variable].squeeze(),
                dt_after,
                dt_before,
            ).squeeze()

            model_results[f"{variable}_rec_err"] = simple_interpolate(
                validation_results_after[f"{variable}_rec_err"].squeeze(),
                validation_results_before[f"{variable}_rec_err"].squeeze(),
                dt_after,
                dt_before,
            ).squeeze()
            for band in np.array(BANDS)[bands_idx.cpu()].tolist():
                model_results[f"{variable}_{band}_rec_err"] = simple_interpolate(
                    validation_results_after[f"{variable}_{band}_rec_err"].squeeze(),
                    validation_results_before[f"{variable}_{band}_rec_err"].squeeze(),
                    dt_after,
                    dt_before,
                ).squeeze()
            model_results[f"{variable}_std"] = simple_interpolate(
                validation_results_after[f"{variable}_std"].squeeze(),
            ...
            results[err_1_le_err_2] = validation_results_before[f"{variable}"].reshape(
                -1
            )[err_1_le_err_2]
            results[np.logical_not(err_1_le_err_2)] = validation_results_after[
                f"{variable}"
            ].reshape(-1)[np.logical_not(err_1_le_err_2)]
            model_results[variable] = results

            date[err_1_le_err_2] = abs(dt_before[err_1_le_err_2])
            date[np.logical_not(err_1_le_err_2)] = abs(
                dt_after[np.logical_not(err_1_le_err_2)]
            )
            model_results[f"{variable}_date"] = date

            results_std[err_1_le_err_2] = validation_results_before[
                f"{variable}_std"
            ].reshape(-1)[err_1_le_err_2]
            results_std[np.logical_not(err_1_le_err_2)] = validation_results_after[
                f"{variable}_std"
            ].reshape(-1)[np.logical_not(err_1_le_err_2)]
            model_results[f"{variable}_std"] = results_std

            results_rec_err[err_1_le_err_2] = validation_results_after[
                f"{variable}_rec_err"
            ].reshape(-1)[err_1_le_err_2]
            results_rec_err[np.logical_not(err_1_le_err_2)] = validation_results_after[
                f"{variable}_rec_err"
            ].reshape(-1)[np.logical_not(err_1_le_err_2)]
            ...
            for band in np.array(BANDS)[bands_idx.cpu()].tolist():
                ...
                results_band_rec_err[err_1_le_err_2] = validation_results_after[
                    f"{variable}_{band}_rec_err"
                ].reshape(-1)[err_1_le_err_2]
                results_band_rec_err[
                    np.logical_not(err_1_le_err_2)
                ] = validation_results_after[f"{variable}_{band}_rec_err"].reshape(-1)[
                    np.logical_not(err_1_le_err_2)
                ]
            ...
            date[np.logical_not(err_1_le_err_2)] = abs(
                dt_before[np.logical_not(err_1_le_err_2)]
            )
            model_results[f"{variable}_date"] = date

            results[err_1_le_err_2] = validation_results_after[f"{variable}"].reshape(
                -1
            )[err_1_le_err_2]
            results[np.logical_not(err_1_le_err_2)] = validation_results_before[
                f"{variable}"
            ].reshape(-1)[np.logical_not(err_1_le_err_2)]
            model_results[variable] = results

            results_std[err_1_le_err_2] = validation_results_after[
                f"{variable}_std"
            ].reshape(-1)[err_1_le_err_2]
            results_std[np.logical_not(err_1_le_err_2)] = validation_results_before[
                f"{variable}_std"
            ].reshape(-1)[np.logical_not(err_1_le_err_2)]
            model_results[f"{variable}_std"] = results_std
            ...
            results_rec_err[np.logical_not(err_1_le_err_2)] = validation_results_before[
                f"{variable}_rec_err"
            ].reshape(-1)[np.logical_not(err_1_le_err_2)]
            model_results[f"{variable}_rec_err"] = results_rec_err
            ...
            for band in np.array(BANDS[bands_idx.cpu()]).tolist():
                ...
                results_band_rec_err[
                    np.logical_not(err_1_le_err_2)
                ] = validation_results_before[f"{variable}_{band}_rec_err"].reshape(-1)[
                    np.logical_not(err_1_le_err_2)
                ]
                model_results[f"{variable}_{band}_rec_err"] = results_band_rec_err
...
def main():
    ...
    output_file_names = compute_frm4veg_data(
        parser.data_dir,
        parser.data_filename,
        s2_product_name,
        date="2018-06-13",
        no_angle_data=False,
    )
