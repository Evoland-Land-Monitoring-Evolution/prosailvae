...
def get_all_campaign_lai_results_BVNET(
    frm4veg_data_dir,
    frm4veg2021_data_dir,
    belsar_data_dir,
    belsar_pred_dir,
    method="simple_interpolate",
    get_all_belsar=False,
    remove_files=False,
    lai_bvnet=None,
):
    all_belsar = None
    ...
    save_bvnet_belsar_predictions(
        belsar_data_dir, belsar_pred_dir, list_belsar_filenames, lai_bvnet=lai_bvnet
    )
    if get_all_belsar:
        all_belsar = get_all_belsar_predictions(
            belsar_data_dir, belsar_pred_dir, "_BVNET"
        )

    barrax_results = interpolate_frm4veg_pred(
        None,
        frm4veg_data_dir,
        BARRAX_FILENAMES[0],
        BARRAX_FILENAMES[1],
    ...
    barrax_2021_results = get_frm4veg_results_at_date(
        None,
        frm4veg2021_data_dir,
        BARRAX_2021_FILENAME,
        is_BVNET=True,
    ...
    wytham_results = interpolate_frm4veg_pred(
        None,
        frm4veg_data_dir,
        WYTHAM_FILENAMES[0],
        WYTHAM_FILENAMES[1],
    ...
    belsar_results = interpolate_belsar_metrics(
        belsar_data_dir=belsar_data_dir,
        belsar_pred_dir=belsar_pred_dir,
        file_suffix="_BVNET",
        method=method,
    )
    if remove_files:
...
def get_all_campaign_CCC_results_BVNET(
    frm4veg_data_dir,
    frm4veg2021_data_dir,
    method="simple_interpolate",
    ccc_bvnet=None,
...
def get_all_campaign_lai_results(
    model,
    frm4veg_data_dir,
    frm4veg2021_data_dir,
    belsar_data_dir,
    ...
    if remove_files:
        for filename in list_belsar_filenames:
            if os.path.isfile(
                os.path.join(belsar_pred_dir, f"{filename}_{model_name}_{mode}.tif")
            ):
                os.remove(
                    os.path.join(belsar_pred_dir, f"{filename}_{model_name}_{mode}.tif")
                )
...
def get_belsar_x_frm4veg_lai_results(
    belsar_results,
    barrax_results,
    barrax_2021_results,
    wytham_results,
    ...
    date_list = [
        belsar_results["date"].values.reshape(-1),
    ...
    ref_lai = np.concatenate(
        [
            belsar_results["ref_lai"].values.reshape(-1),
    ...
    ref_lai_std_list = [
        belsar_results["ref_lai_std"].values.reshape(-1),
    ...
    pred_lai_list = [
        belsar_results["lai_mean"].values.reshape(-1),
    ...
    pred_lai_std_list = [
        belsar_results["lai_sigma_mean"].values.reshape(-1),
    ...
    land_cover_list = [
        belsar_results["land_cover"].values.reshape(-1),
    ...
    if get_reconstruction_error:
        rec_err = np.concatenate(
            [
                belsar_results["rec_err_mean"].values.reshape(-1),
    ...
    for band in np.array(BANDS)[bands_idx.cpu()].tolist():
        if get_reconstruction_error:
            bands_rec_err[band] = np.concatenate(
                [
                    belsar_results[f"{band}_rec_err_mean"].values.reshape(-1),
...
def get_frm4veg_ccc_results(
    ...
    if bands_idx is None:
        bands_idx = torch.arange(10)
    date_list = [
        barrax_results[f"{frm4veg_ccc}_date"].reshape(-1),
    ...
    if get_reconstruction_error:
        rec_err = np.concatenate(
            [
                barrax_results[f"{frm4veg_ccc}_rec_err"].reshape(-1),
    ...
    for band in np.array(BANDS)[bands_idx.cpu()].tolist():
        if get_reconstruction_error:
            bands_rec_err[band] = np.concatenate(
                [
                    barrax_results[f"{frm4veg_ccc}_{band}_rec_err"].reshape(-1),
...
def get_validation_global_metrics(
    df_results,
    decompose_along_columns=None,
    n_sigma=3,
    variable="lai",
):
    if decompose_along_columns is None:
    ...
    for column in decompose_along_columns:
        ...
        for _, element in enumerate(pd.unique(df_results[column])):
            ...
            rmse[f"{variable}_rmse_{element}"] = np.sqrt(
                (results[f"Predicted {variable}"] - results[variable]).pow(2).mean()
            )
        rmse[f"{variable}_rmse_all"] = np.sqrt(
            (df_results[f"Predicted {variable}"] - df_results[variable]).pow(2).mean()
        )
    ...
    if n_sigma > 0:
        for column in decompose_along_columns:
            ...
            for _, element in enumerate(pd.unique(df_results[column])):
                results = df_results[df_results[column] == element]
                picp[f"{variable}_picp_{element}"] = (
                    np.logical_and(
                        results[variable]
                        < results[f"Predicted {variable}"]
                        + n_sigma * results[f"Predicted {variable} std"],
                        results[variable]
                        > results[f"Predicted {variable}"]
                        - n_sigma * results[f"Predicted {variable} std"],
                    )
            ...
            picp[f"{variable}_picp_all"] = (
                np.logical_and(
                    df_results[variable]
                    < df_results[f"Predicted {variable}"]
                    + n_sigma * df_results[f"Predicted {variable} std"],
                    df_results[variable]
                    > df_results[f"Predicted {variable}"]
                    - n_sigma * df_results[f"Predicted {variable} std"],
                )
    ...
    if n_sigma > 0:
        for column in decompose_along_columns:
            ...
            for _, element in enumerate(pd.unique(df_results[column])):
                results = df_results[df_results[column] == element]
                mpiw[f"{variable}_mpiw_{element}"] = (
                    2 * n_sigma * results[f"Predicted {variable} std"]
                ).mean()
            mpiw[f"{variable}_mpiw_all"] = (
                2 * n_sigma * df_results[f"Predicted {variable} std"]
            ).mean()
    ...
    for column in decompose_along_columns:
        ...
        for _, element in enumerate(pd.unique(df_results[column])):
            results = df_results[df_results[column] == element]
            mestdr[f"{variable}_mestdr_{element}"] = (
                np.abs(results[variable] - results[f"Predicted {variable}"])
                / results[f"Predicted {variable} std"]
            ).mean()
        mestdr[f"{variable}_mestdr_all"] = (
            np.abs(df_results[variable] - df_results[f"Predicted {variable}"])
            / df_results[f"Predicted {variable} std"]
        ).mean()
