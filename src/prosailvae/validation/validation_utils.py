...
def read_data_from_theia(
    left, bottom, right, top, src_epsg, path_to_theia_product, margin=100
):
    dataset = sentinel2.Sentinel2(path_to_theia_product)
    left, bottom, right, top = rio.warp.transform_bounds(
        src_epsg, dataset.crs.to_epsg(), left, bottom, right, top
    )
    ...
    validity_mask = (
        np.sum(masks.data, axis=0, keepdims=True).astype(bool).astype(int).astype(float)
    )
    s2_r = s2_r.data
