...
def raster_time_serie(raster_path: str | Path) -> pd.DataFrame:
    """
    ...
    raster_filenames = [
        filename
        for filename in glob.glob(f"{raster_path}/SENTINEL2*.tif")
    ...
    ts_df["s2_date"] = (
        ts_df["s2_filenames"].apply(get_acquisition_date).apply(pd.to_datetime)
    )
    return ts_df
...
def get_pixel_value(
    ...
    # get coordinates
    coord_list = [
        (x, y) for x, y in zip(pixel_coords["geometry"].x, pixel_coords["geometry"].y)
    ]
    # extract pixel values
    with rio.open(raster_filename) as raster:
        pixel_values = [coords for coords in raster.sample(coord_list)]
    # araay
    ...
    s2_array[:, 11] = np.logical_or(
        ...
        np.logical_or(
            s2_array[:, 10].astype(np.uint8), s2_array[:, 12].astype(np.uint8)
        ),
    )
    # compute the angles
...
def compute_time_deltas(
    ...
    """
    time_delta = [
        vector_timestamp - raster_ts for raster_ts in raster_timestamp.to_list()
    ]
    return time_delta
...
def get_pixels(n_nearest, vector_ts, vector_ds, col_names):
    ...
    s2_pix_values["s2_filenames"] = [
        n_nearest[1]["s2_filenames"] for i in range(len(s2_pix_values))
    ]
    s2_pix_values["time_delta"] = [
        n_nearest[1]["time_delta"] for i in range(len(s2_pix_values))
    ]
    s2_pix_values["s2_date"] = [
        n_nearest[1]["s2_date"] for i in range(len(s2_pix_values))
    ]
    values_np = np.concatenate((np.array(s2_pix_values), np.array(vector_ds)), axis=1)
    values_df = pd.DataFrame(
        values_np,
        columns=list(*chain([list(s2_pix_values.columns) + (list(vector_ds.columns))])),
    )
    return values_df


def check_pixel_validity(
    values_df: gpd.GeoDataFrame,
) -> gpd.GeoDataFrame:
    """
    Check if the pixel are valids
    """
    values_df["sat_mask"] = values_df["sat_mask"].apply(lambda x: int(x)).to_list()
    values_df["cloud_mask"] = values_df["cloud_mask"].apply(lambda x: int(x)).to_list()
    values_df["edge_mask"] = values_df["edge_mask"].apply(lambda x: int(x)).to_list()
    values_df["geophysical_mask"] = (
        values_df["geophysical_mask"].apply(lambda x: int(x)).to_list()
    )
...
def clean_france(
    ...
    vector_ds["field_date"] = (
        vector_ds["Date DHP"].apply(normalize_france_date).apply(pd.to_datetime)
    )
...
def clean_spain(
    ...
    vector_ds["field_date"] = (
        vector_ds["f_datetime"].apply(normalize_spain_date).apply(pd.to_datetime)
    )
...
def clean_italy(
    ...
    vector_ds["field_date"] = (
        vector_ds["Date"].apply(normalize_italy_date).apply(pd.to_datetime)
    )
...
def main(args: argparse.ArgumentParser):
    ...
    window_days = 25
    for vector_ts, vector_ds in tqdm(
        vector_data.groupby("field_date"), desc="Extracting Pixel Values :"
    ):
        ...
        raster_ts["time_delta_after"] = raster_ts["time_delta"][
            raster_ts["time_delta"] >= timedelta(days=0)
        ]
        raster_ts["time_delta_before"] = raster_ts["time_delta"][
            raster_ts["time_delta"] < timedelta(days=0)
        ]
    ...
    if len(after_values_list):
        ...
        logging.info(
            f"Exported to : {args.export_path}/after_{config.site}_{config.raster.split('/')[-2]}.csv"
        )
