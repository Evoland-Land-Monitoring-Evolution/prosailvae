...
from torch.distributions import Uniform, register_kl
...
class TruncatedNormal(Distribution):
    ...
    def __init__(self, loc, scale, low, high, validate_args=None):
        ...
        if isinstance(low, Number) and isinstance(high, Number) and isinstance(loc, Number) and isinstance(scale, Number):
        ...
        if any((self.low >= self.high).view(-1,).tolist()):
...
def kl_truncated_normal_truncated_normal(p, q):
    ...
    kl_pq += - (p.scaled_low * p.little_phi_a - p.scaled_high * p.little_phi_b) / (2 * p.Z) * (1 - sq_sigma1 / sq_sigma2)
    kl_pq += (p.loc - q.loc).pow(2) / 2 / sq_sigma2
    kl_pq += sq_sigma1 / 2 / sq_sigma2
    kl_pq += (p.loc - q.loc) * p.scale / p.Z / sq_sigma2 * (p.little_phi_a - p.little_phi_b)
    return kl_pq
...
def numerical_kl(density_p: torch.Tensor, density_q: torch.Tensor, dim: int=0, dx=1):
    ...
    kl_pq = (density_p * density_p.log()).sum(dim) - (density_p * density_q.log()).sum(dim)
    return kl_pq * dx
...
def test_kl_u():
    ...
    density_p = p.pdf(support) #Exclude last value for uniform density (high bound exluded)
