...
def get_belsar_x_frm4veg_lai_results(belsar_results, barrax_results, wytham_results,
    ...
    pred_lai_std_list = [belsar_results['lai_sigma_mean'].values.reshape(-1),
                            barrax_results[frm4veg_lai + "_std"].reshape(-1)]
    ...
    ref_lai_list.append(wytham_results[f'ref_{frm4veg_lai}'].reshape(-1))
    ref_lai_std_list.append(wytham_results[f'ref_{frm4veg_lai}_std'].reshape(-1))
    pred_lai_list.append(wytham_results[frm4veg_lai].reshape(-1))
    pred_lai_std_list.append(wytham_results[f"{frm4veg_lai}_std"].reshape(-1))
    site_list = site_list + ['England'] * len(ref_lai_list[2])
    land_cover_list.append(wytham_results[f'{frm4veg_lai}_land_cover'].reshape(-1))
